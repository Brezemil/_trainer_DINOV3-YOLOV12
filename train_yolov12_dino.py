...
def create_model_config_path(yolo_size, dinoversion=None, dino_variant=None, integration=None, dino_input=None):
    ...
    if not config_path.exists():
        ...
        if integration == "dual":
            ...
            systematic_config = (
                f"ultralytics/cfg/models/v12/yolov12{yolo_size}-dino{dinoversion}-{dino_variant}-dualp0p3.yaml"
            )
            if Path(systematic_config).exists():
                print(
                    f"   📄 Using systematic dualp0p3 config: yolov12{yolo_size}-dino{dinoversion}-{dino_variant}-dualp0p3.yaml"
                )
                return systematic_config
            # Try legacy naming pattern
            legacy_config = (
                f"ultralytics/cfg/models/v12/yolov12{yolo_size}-dualp0p3-dino{dinoversion}-{dino_variant}.yaml"
            )
            if Path(legacy_config).exists():
                print(
                    f"   📄 Using legacy dualp0p3 config: yolov12{yolo_size}-dualp0p3-dino{dinoversion}-{dino_variant}.yaml"
                )
            ...
            if Path(fallback_config).exists():
                print(
                    f"   📄 Using dual config as base template for dualp0p3 fallback: yolov12{yolo_size}-dino3-vitb16-dual.yaml"
                )
...
def parse_arguments():
    ...
    # Required arguments
    parser.add_argument("--data", type=str, required=True, help="Dataset YAML file path (e.g., coco.yaml)")
    parser.add_argument(
        "--yolo-size", type=str, required=True, choices=["n", "s", "m", "l", "x"], help="YOLOv12 model size (n/s/m/l/x)"
    )
    ...
    parser.add_argument(
        "--dino-input", type=str, default=None, help="Custom DINO model input/path (overrides --dino-variant)"
    )
    parser.add_argument("--pretrain", type=str, default=None, help="Pretrained YOLO checkpoint to load (.pt file)")
    parser.add_argument(
        "--pretrainyolo",
        type=str,
        default=None,
    ...
    # Training parameters
    parser.add_argument(
        "--epochs", type=int, default=None, help="Number of training epochs (auto-determined if not specified)"
    )
    parser.add_argument("--batch-size", type=int, default=None, help="Batch size (auto-determined if not specified)")
    ...
    parser.add_argument(
        "--warmup-bias-lr", type=float, default=0.1, help="Warmup initial bias learning rate (default: 0.1)"
    )
    ...
    parser.add_argument(
        "--momentum", type=float, default=0.937, help="SGD momentum/beta1 for Adam optimizers (default: 0.937)"
    )
    parser.add_argument("--weight-decay", type=float, default=0.0005, help="Weight decay (default: 0.0005)")
    parser.add_argument("--kobj", type=float, default=1.0, help="Objectness loss gain (default: 1.0)")

    # Regularization
    parser.add_argument("--label-smoothing", type=float, default=0.0, help="Label smoothing epsilon (default: 0.0)")
    parser.add_argument("--dropout", type=float, default=0.0, help="Dropout rate for training (default: 0.0)")
    parser.add_argument(
        "--grad-clip", type=float, default=0.0, help="Gradient clipping max norm (0 to disable, default: 0.0)"
    )

    # Data augmentation parameters
    parser.add_argument("--scale", type=float, default=0.5, help="Image scale augmentation (default: 0.5)")
    ...
    parser.add_argument(
        "--copy-paste", type=float, default=0.0, help="Copy-paste augmentation probability (default: 0.0)"
    )

    # Advanced augmentation controls
    parser.add_argument(
        "--hsv-h", type=float, default=0.015, help="HSV-Hue augmentation range (fraction) (default: 0.015)"
    )
    parser.add_argument(
        "--hsv-s", type=float, default=0.7, help="HSV-Saturation augmentation range (fraction) (default: 0.7)"
    )
    parser.add_argument(
        "--hsv-v", type=float, default=0.4, help="HSV-Value augmentation range (fraction) (default: 0.4)"
    )
    parser.add_argument("--degrees", type=float, default=0.0, help="Rotation degrees (default: 0.0)")
    parser.add_argument(
        "--translate", type=float, default=0.1, help="Translation range as fraction of image size (default: 0.1)"
    )
    parser.add_argument("--shear", type=float, default=0.0, help="Shear degrees (default: 0.0)")
    parser.add_argument(
        "--perspective", type=float, default=0.0, help="Perspective transformation coefficient (default: 0.0)"
    )
    ...
    parser.add_argument("--erasing", type=float, default=0.4, help="Random erasing probability (default: 0.4)")
    parser.add_argument(
        "--crop-fraction", type=float, default=1.0, help="Image crop fraction for classification (default: 1.0)"
    )

    # Training control options
    parser.add_argument("--resume", type=str, default=None, help="Resume training from checkpoint")
    parser.add_argument(
        "--save-period", type=int, default=-1, help="Save checkpoint every n epochs (-1 to disable, default: -1)"
    )
    parser.add_argument("--val", action="store_true", default=True, help="Validate during training (default: True)")
    parser.add_argument("--plots", action="store_true", default=True, help="Generate training plots (default: True)")
    parser.add_argument("--patience", type=int, default=100, help="Early stopping patience (epochs) (default: 100)")
    parser.add_argument(
        "--close-mosaic", type=int, default=10, help="Disable mosaic augmentation for final epochs (default: 10)"
    )

    # Evaluation and results display options
    parser.add_argument(
        "--eval-test", action="store_true", default=True, help="Evaluate on test dataset if available (default: True)"
    )
    parser.add_argument(
        "--eval-val", action="store_true", default=True, help="Evaluate on validation dataset (default: True)"
    )
    parser.add_argument(
        "--detailed-results",
        action="store_true",
    ...
    parser.add_argument("--save-results-table", action="store_true", help="Save results summary as markdown table")

    # Loss function parameters
    parser.add_argument("--box", type=float, default=7.5, help="Box loss gain (default: 7.5)")
    parser.add_argument("--cls", type=float, default=0.5, help="Classification loss gain (default: 0.5)")
    parser.add_argument("--dfl", type=float, default=1.5, help="Distribution focal loss gain (default: 1.5)")
    ...
    parser.add_argument("--fraction", type=float, default=1.0, help="Dataset fraction for training (default: 1.0)")
    parser.add_argument("--profile", action="store_true", help="Profile ONNX and TensorRT speeds during validation")
...
def modify_yaml_config_for_custom_dino(config_path, dino_input, yolo_size="s", unfreeze_dino=False, dino_version="3"):
    ...
    with open(temp_path, "r") as f:
        ...
        # Find and show lines around DINO3Preprocessor or DINO3Backbone
        for i, line in enumerate(lines):
            if "DINO3Preprocessor" in line or "DINO3Backbone" in line:
                start = max(0, i - 2)
                end = min(len(lines), i + 5)
                dino_type = "DINO3Preprocessor" if "DINO3Preprocessor" in line else "DINO3Backbone"
                print(f"   Found {dino_type} around line {i + 1}:")
            ...
            try:
                ...
                if current_fraction < 1.0:
                    ...
                    if adjusted_fraction != current_fraction:
                        print(
                            f"⚠️  Dataset fraction {current_fraction} resulted in empty validation set. "
