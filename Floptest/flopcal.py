...
def parse_arguments():
    ...
    parser.add_argument(
        "--dinoversion", type=str, choices=["2", "3"], default=None, help="DINO version (2 for DINOv2, 3 for DINOv3)"
    )
    parser.add_argument(
        "--dino-variant",
        type=str,
    ...
    parser.add_argument("--dino-input", type=str, default=None, help="Custom DINO model input/path")

    # Model parameters
    parser.add_argument("--imgsz", type=int, default=640, help="Input image size (default: 640)")
    parser.add_argument("--batch-size", type=int, default=1, help="Batch size for FLOPs calculation (default: 1)")
    parser.add_argument("--device", type=str, default="cpu", help="Device for calculation (cpu/cuda)")
    parser.add_argument(
        "--unfreeze-dino", action="store_true", help="Make DINO backbone trainable (affects parameter count)"
    )
    ...
    parser.add_argument("--detailed", action="store_true", help="Show detailed layer-wise breakdown")
    parser.add_argument("--save-report", type=str, default=None, help="Save report to file (markdown format)")
...
def validate_arguments(args):
    ...
    if dino_requested:
        ...
        if not args.dino_input and args.dino_variant:
            args.dino_input = f"dinov{args.dinoversion}_{args.dino_variant}"
    else:
        if args.integration:
...
def generate_report(args, results, model_info):
    ...
    if "flops_thop" in results and results["flops_thop"] is not None:
        ...
        report.append("")
        report.append(
            "*Note: These are theoretical estimates and actual performance will vary based on memory bandwidth, optimization, and implementation.*"
        )
