...
def restore_exact_training_state(checkpoint_path):
    ...
    try:
        print("🎯 RESTORING EXACT TRAINING STATE")
        print("=" * 60)
        ...
        print("📊 Complete Training State Extracted:")
        ...
        if "train_results" in checkpoint and checkpoint["train_results"]:
            ...
            if expected_losses:
                print("   Expected Loss Values (training should start with these):")
...
def create_resume_model(checkpoint_path):
    ...
    try:
        ...
        print("🔍 Extracting original architecture from checkpoint...")
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        ...
        if "train_args" in checkpoint:
            ...
            if original_config and Path(original_config).exists():
                ...
                print("🔧 Falling back to direct checkpoint loading...")
                model = YOLO(checkpoint_path)
        else:
            print("⚠️  No train_args in checkpoint, using direct loading...")
            model = YOLO(checkpoint_path)
        ...
        print("🔍 Verifying architecture compatibility...")
        if "train_metrics" in checkpoint:
            ...
            print("📊 Expected loss values from checkpoint:")
            print(f"   box_loss: {expected_box_loss}")
            print(f"   cls_loss: {expected_cls_loss}")
            print(f"   dfl_loss: {expected_dfl_loss}")
            print("🎯 Training should start with similar loss values!")
            print("⚠️  If initial loss is 4x higher, there's still an architecture mismatch!")
        ...
        if dino_params > 0:
            ...
            if frozen_count > 0:
                ...
                print("🧊 DINO parameters already frozen")
...
def parse_arguments():
    ...
    parser.add_argument(
        "--epochs", type=int, default=None, help="Number of epochs to train (default: continue from checkpoint)"
    )
    parser.add_argument("--data", type=str, default=None, help="Dataset YAML file (default: use checkpoint's dataset)")
    parser.add_argument(
        "--batch-size", type=int, default=None, help="Batch size (default: use checkpoint's batch size)"
    )
    parser.add_argument("--device", type=str, default=None, help="Device (default: auto-detect)")
    parser.add_argument("--name", type=str, default=None, help="Experiment name (default: auto-generate)")

    # Hyperparameter overrides
    parser.add_argument("--lr", type=float, default=None, help="Learning rate override")
    parser.add_argument(
        "--optimizer", type=str, default=None, choices=["SGD", "Adam", "AdamW"], help="Optimizer override"
    )
    parser.add_argument("--patience", type=int, default=None, help="Early stopping patience")

    # Advanced options
    parser.add_argument(
        "--unfreeze-dino", action="store_true", help="Unfreeze DINO weights for fine-tuning (only for DINO models)"
    )
    parser.add_argument(
        "--resume-mode",
        type=str,
    ...
    parser.add_argument("--amp", type=bool, default=None, help="Enable/disable AMP (default: auto for model type)")
...
def extract_all_hyperparameters(checkpoint_path):
    ...
    print("🔧 EXTRACTING ALL HYPERPARAMETERS FROM CHECKPOINT")
    print("=" * 60)
    ...
    print("📈 FINAL TRAINING STATE RESTORATION:")
    ...
    if hasattr(train_results, "get") and "lr/pg0" in train_results:
        ...
        if final_lr_list and len(final_lr_list) > 0:
            ...
            print("   🎯 Training continues with NO warmup - exact final parameters!")
    ...
    print("   Warmup disabled: warmup_epochs=0 (continuing from final state)")
    ...
    if "dino" in original_model.lower():
        ...
        print("⚡ Auto-disabled AMP for DINO model")
...
def validate_arguments(args, analysis):
    ...
    if args.data is None:
        print("⚠️  No dataset specified! Please provide --data parameter")
    ...
    print("🎯 Training will use IDENTICAL settings as original training")
...
def resume_training(args, analysis, model, training_state):
    ...
    try:
        ...
        if args.unfreeze_dino and analysis["is_dino"]:
            print("🔥 Unfreezing DINO weights for fine-tuning...")
            unfrozen_count = 0
            for name, param in model.model.named_parameters():
        ...
        print("   This will restore: optimizer state, EMA weights, epoch counter, learning rate scheduler")
        ...
        print("🎯 YOLO will automatically restore:")
        print("   ✅ Model weights (including EMA)")
        print("   ✅ Optimizer state (momentum buffers, etc.)")
        print("   ✅ Learning rate scheduler state")
        print("   ✅ Epoch counter (+1 from checkpoint)")
        print("   ✅ Best fitness tracking")
        ...
        print("🎯 Training with EXACT state continuation...")
        print(f"   Checkpoint: {args.checkpoint}")
        ...
        print("🚀 Starting training with NO warnings and EXACT continuation...")
