...
`compute_visualization`(samples,Â ...) | See `fiftyone/brain/__init__.py`.  
---|---  
`values`(results,Â path_or_expr) |   
`visualize`(results[,Â labels,Â sizes,Â classes,Â ...]) |   
  
**Classes:**

`VisualizationResults`(samples,Â config,Â ...[,Â ...]) | Class storing the results of [`fiftyone.brain.compute_visualization()`](https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_visualization "fiftyone.brain.compute_visualization").  
---|---  
`VisualizationConfig`([embeddings_field,Â ...]) | Base class for configuring visualization methods.  
`Visualization`(config) |   
`UMAPVisualizationConfig`([embeddings_field,Â ...]) | Configuration for Uniform Manifold Approximation and Projection (UMAP) embedding visualization.  
`UMAPVisualization`(config) |   
`TSNEVisualizationConfig`([embeddings_field,Â ...]) | Configuration for t-distributed Stochastic Neighbor Embedding (t-SNE) visualization.  
`TSNEVisualization`(config) |   
`PCAVisualizationConfig`([embeddings_field,Â ...]) | Configuration for principal component analysis (PCA) embedding visualization.  
`PCAVisualization`(config) |   
`ManualVisualizationConfig`([patches_field,Â ...]) | Configuration for manually-provided low-dimensional visualizations.  
...
Parameters:
  ...
  * **config** â the `VisualizationConfig` used

  * **brain_key** â the brain key

  * **points** â a `num_points x num_dims` array of visualization points

  * **sample_ids** (_None_) â a `num_points` array of sample IDs

  * **label_ids** (_None_) â a `num_points` array of label IDs, if applicable

  * **backend** (_None_) â a `Visualization` backend
...
`use_view`(sample_collection[,Â allow_missing,Â ...]) | Restricts the index to the provided view.  
---|---  
...
`visualize`([labels,Â sizes,Â classes,Â backend]) | Generates an interactive scatterplot of the visualization results for the current `view()`.  
`index_points`([points_field,Â create_index,Â ...]) | Adds a spatial index for these visualization results to its dataset's samples.  
...
data to use to scale the sizes of the points. Can be any of the following:
  ...
  * **classes** (_None_) â an optional list of classes whose points to plot. Only applicable when `labels` contains strings
...
Adds a spatial index for these visualization results to its datasetâs samples.

This method is useful if you want to add a spatial index to existing visualization results that donât yet have one.
...
Parameters:
  ...
  * **points_field** (_None_) â an optional field name in which to store the spatial index. The default is the resultâs `brain_key`

  * **create_index** (_True_) â whether to create a database index for the points
...
Parameters:
  ...
  * **points_field** (_None_) â the name of a field in which to store the visualization points, if requested

  * **similarity_index** (_None_) â the similarity index containing the embeddings, if one was provided
  ...
  * **num_dims** (_2_) â the dimension of the visualization space
...
Parameters:
  ...
  * **num_neighbors** (_15_) â the number of neighboring points used in local approximations of manifold structure. Larger values will result in more global structure being preserved at the loss of detailed local structure. Typical values are in `[5, 50]`

  * **metric** (_"euclidean"_) â the metric to use when calculating distance between embeddings. See the UMAP documentation for supported values

  * **min_dist** (_0.1_) â the effective minimum distance between embedded points. This controls how tightly the embedding is allowed compress points together. Larger values ensure embedded points are more evenly distributed, while smaller values allow the algorithm to optimise more accurately with regard to local structure. Typical values are in `[0.001, 0.5]`

  * **seed** (_None_) â a random seed

  * **verbose** (_True_) â whether to log progress
...
Parameters:
  ...
  * **pca_dims** (_50_) â the number of PCA dimensions to compute prior to running t-SNE. It is highly recommended to reduce the number of dimensions to a reasonable number (e.g. 50) before running t-SNE, as this will suppress some noise and speed up the computation of pairwise distances between samples

  * **svd_solver** (_"randomized"_) â the SVD solver to use when performing PCA. Consult the sklearn docmentation for details

  * **metric** (_"euclidean"_) â the metric to use when calculating distance between embeddings. Must be a supported value for the `metric` argument of `scipy.spatial.distance.pdist`

  * **perplexity** (_30.0_) â the perplexity to use. Perplexity is related to the number of nearest neighbors that is used in other manifold learning algorithms. Larger datasets usually require a larger perplexity. Typical values are in `[5, 50]`

  * **learning_rate** (_200.0_) â the learning rate to use. Typical values are in `[10, 1000]`. If the learning rate is too high, the data may look like a ball with any point approximately equidistant from its nearest neighbours. If the learning rate is too low, most points may look compressed in a dense cloud with few outliers. If the cost function gets stuck in a bad local minimum increasing the learning rate may help

  * **max_iters** (_1000_) â the maximum number of iterations to run. Should be at least 250
...
Parameters:
  ...
  * **svd_solver** (_"randomized"_) â the SVD solver to use. Consult the sklearn docmentation for details
