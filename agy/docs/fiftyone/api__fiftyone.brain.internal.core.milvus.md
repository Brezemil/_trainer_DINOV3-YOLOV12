---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.milvus.html
---
...
`MilvusSimilarityConfig`([collection_name,Â ...]) | Configuration for the Milvus similarity backend.  
---|---  
`MilvusSimilarity`(config) | Milvus similarity factory.  
`MilvusSimilarityIndex`(samples,Â config,Â brain_key) | Class for interacting with Milvus similarity indexes.  
  
class fiftyone.brain.internal.core.milvus.MilvusSimilarityConfig(_collection_name =None_, _metric ='dotproduct'_, _consistency_level ='Session'_, _uri =None_, _user =None_, _password =None_, _secure =None_, _token =None_, _db_name =None_, _client_key_path =None_, _client_pem_path =None_, _ca_pem_path =None_, _server_pem_path =None_, _server_name =None_, _** kwargs_)#
...
Parameters:
  ...
  * **collection_name** (_None_) â the name of a Milvus collection to use or create. If none is provided, a new collection will be created

  * **metric** (_"dotproduct"_) â the embedding distance metric to use when creating a new index. Supported values are `("cosine", "dotproduct", "euclidean")`

  * **consistency_level** (_"Session"_) â the consistency level to use. Supported values are `("Session", "Strong", "Bounded", "Eventually")`

  * **uri** (_None_) â a full Milvus server address to use, like `"http://localhost:19530"`, `"tcp:localhost:19530"`, or `"https://ok.s3.south.com:19530"`

  * **user** (_None_) â a username to use

  * **password** (_None_) â a password to use

  * **secure** (_None_) â whether to enable TLS (True)

  * **token** (_None_) â a header token for RPC calls

  * **db_name** (_None_) â a database name for the connection

  * **client_key_path** (_None_) â a client.key path for TLS two-way

  * **client_pem_path** (_None_) â a client.pem path for TLS two-way

  * **ca_pem_path** (_None_) â a ca.pem path for TLS two-way

  * **server_pem_path** (_None_) â a server.pem path for TLS one-way

  * **server_name** (_None_) â the server name, for TLS
...
`load_credentials`([uri,Â user,Â password,Â ...]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
property client_key_path#
    

property client_pem_path#
    

property ca_pem_path#
    

property server_pem_path#
...
load_credentials(_uri =None_, _user =None_, _password =None_, _secure =None_, _token =None_, _db_name =None_, _client_key_path =None_, _client_pem_path =None_, _ca_pem_path =None_, _server_pem_path =None_, _server_name =None_)#
...
class fiftyone.brain.internal.core.milvus.MilvusSimilarity(_config_)#
...
**config** â a `MilvusSimilarityConfig`
...
class fiftyone.brain.internal.core.milvus.MilvusSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `MilvusSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `MilvusSimilarity` instance
...
`collection` | The `pymilvus.Collection` instance for this index.  
---|---  
...
add_to_index(_embeddings_ , _sample_ids_ , _label_ids =None_, _overwrite =True_, _allow_existing =True_, _warn_existing =False_, _reload =True_, _batch_size =100_)#
