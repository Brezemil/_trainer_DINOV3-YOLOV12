---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/hf_fine_tuner_plugin.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/hf_fine_tuner_plugin)
...
![image](https://raw.githubusercontent.com/harpreetsahota204/hf_fine_tuner_plugin/main/icons/ft_classification.gif)
...
![image](https://raw.githubusercontent.com/harpreetsahota204/hf_fine_tuner_plugin/main/icons/ft_detection.gif)
...
---|---|---|---  
`label_field` | str | _required_ | The label field to train on  
`model_name` | str | _required_ | HuggingFace model ID  
...
Training runs as a delegated operation in the App by default. To use delegated execution from the SDK, pass `delegate=True`.
...
By default, training runs on a single GPU to avoid issues with PyTorchâs `DataParallel` (which breaks models like DETR). If you want to use a different GPU, set `CUDA_VISIBLE_DEVICES` before launching the delegated service (e.g. `export CUDA_VISIBLE_DEVICES=1`). For multi-GPU training, use an `accelerate`-compatible setup.
