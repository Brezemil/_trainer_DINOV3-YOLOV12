---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/active_learning.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/jacobmarks/active-learning-plugin)
...
What if you could make this process smarter and more efficient? Enter [Active Learning](https://en.wikipedia.org/wiki/Active_learning_machine_learning) â a paradigm that iteratively selects the most âinformativeâ or âambiguousâ examples for labeling, thereby reducing the amount of manual annotation needed. In practical terms, this means your model gets better, faster, and with fewer labeled samples.

This FiftyOne plugin brings Active Learning to your computer vision data using the [modAL](https://modal-python.readthedocs.io/en/latest/) library, allowing you to integrate this accelerant directly into your annotation workflow. Now you can prioritize, query, and annotate the most crucial data points, all within the FiftyOne Appâno coding necessary.

The best part? You can use this in tandem with your traditional annotation service providers (via FiftyOneâs integrations with CVAT, Labelbox and Label Studio), or even with the FiftyOne [Zero-shot Prediction plugin](https://github.com/jacobmarks/zero-shot-prediction-plugin)!
...
We can choose:
  ...
  * The default batch size â the number of samples per query
...
Executing this operator creates a modAL `ActiveLearner` that uses an âuncertaintyâ batch sampling. The execution also invokes the generation of initial predictions, and triggers the reload of the dataset.

## `query_learner`#

Queries the active learner for the next samples to label. If youâd like, you can override the default query batch size.
...
After correcting the incorrect query labels, we can update our active learner by âteachingâ it this new information. Running this operator updates our active learning model, updates the label field with new predictions, and reloads the app.
...
Now weâre ready to create an active learner. We can do this using the `create_active_learner` operator:
...
Once weâve created an active learner, we can query it for the next batch of samples to label. We can do this using the `query_learner` operator:
...
After correcting the incorrect query labels, we can update our active learner by âteachingâ it this new information. We can do this using the `update_learner_predictions` operator:
...
Now we can repeat steps 3 and 4 until weâre satisfied with our modelâs performance.
