---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.pinecone.html
---
...
`PineconeSimilarityConfig`([index_name,Â ...]) | Configuration for the Pinecone similarity backend.  
---|---  
`PineconeSimilarity`(config) | Pinecone similarity factory.  
`PineconeSimilarityIndex`(samples,Â config,Â ...) | Class for interacting with Pinecone similarity indexes.  
  
class fiftyone.brain.internal.core.pinecone.PineconeSimilarityConfig(_index_name =None_, _index_type =None_, _namespace =None_, _metric =None_, _replicas =None_, _shards =None_, _pods =None_, _pod_type =None_, _api_key =None_, _cloud =None_, _region =None_, _environment =None_, _** kwargs_)#
...
Parameters:
  ...
  * **index_name** (_None_) â the name of a Pinecone index to use or create. If none is provided, a new index will be created

  * **index_type** (_None_) â the index type to use when creating a new index. The supported values are `["serverless", "pod"]` and the default is `"serverless"`

  * **namespace** (_None_) â a namespace under which to store vectors added to the index

  * **metric** (_None_) â the embedding distance metric to use when creating a new index. Supported values are `("cosine", "dotproduct", "euclidean")`

  * **replicas** (_None_) â an optional number of replicas when creating a new pod-based index

  * **shards** (_None_) â an optional number of shards when creating a new pod-based index

  * **pods** (_None_) â an optional number of pods when creating a new pod-based index

  * **pod_type** (_None_) â an optional pod type when creating a new pod-based index

  * **api_key** (_None_) â a Pinecone API key to use

  * **cloud** (_None_) â a cloud to use when creating serverless indexes

  * **region** (_None_) â a region to use when creating serverless indexes

  * **environment** (_None_) â an environment to use when creating pod-based indexes
...
`load_credentials`([api_key,Â cloud,Â region,Â ...]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
load_credentials(_api_key =None_, _cloud =None_, _region =None_, _environment =None_)#
...
class fiftyone.brain.internal.core.pinecone.PineconeSimilarity(_config_)#
...
**config** â a `PineconeSimilarityConfig`
...
class fiftyone.brain.internal.core.pinecone.PineconeSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `PineconeSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `PineconeSimilarity` instance
...
`index` | The `pinecone.Index` instance for this index.  
---|---  
...
add_to_index(_embeddings_ , _sample_ids_ , _label_ids =None_, _overwrite =True_, _allow_existing =True_, _warn_existing =False_, _reload =True_, _batch_size =100_, _namespace =None_)#
