---
source_url: https://docs.voxel51.com/api/fiftyone.utils.clip.model.html
---

# fiftyone.utils.clip.model#

CLIP model from [openai/CLIP](https://github.com/openai/CLIP).
...
`Bottleneck`(inplanes,Â planes[,Â stride]) |   
---|---  
`AttentionPool2d`(spacial_dim,Â embed_dim,Â ...) |   
`ModifiedResNet`(layers,Â output_dim,Â heads[,Â ...]) | A ResNet class that is similar to torchvision's but contains the following changes:  
`LayerNorm`(normalized_shape[,Â eps,Â ...]) | Subclass torch's LayerNorm to handle fp16.  
`QuickGELU`(*args,Â **kwargs) |   
`ResidualAttentionBlock`(d_model,Â n_head[,Â ...]) |   
`Transformer`(width,Â layers,Â heads[,Â attn_mask]) |   
`VisionTransformer`(input_resolution,Â ...) |   
`CLIP`(embed_dim,Â image_resolution,Â ...) |   
  
**Functions:**

`convert_weights`(model) | Converts applicable model parameters to fp16.  
---|---  
`build_model`(state_dict) |   
  
class fiftyone.utils.clip.model.Bottleneck(_inplanes_ , _planes_ , _stride =1_)#
...
forward(_x : [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor "\(in PyTorch v2.12\)")_)#
...
class fiftyone.utils.clip.model.AttentionPool2d(_spacial_dim : int_, _embed_dim : int_, _num_heads : int_, _output_dim : int = None_)#
...
class fiftyone.utils.clip.model.ModifiedResNet(_layers_ , _output_dim_ , _heads_ , _input_resolution =224_, _width =64_)#
    

Bases: [`Module`](https://docs.pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "\(in PyTorch v2.12\)")

A ResNet class that is similar to torchvisionâs but contains the following changes:

  * There are now 3 âstemâ convolutions as opposed to 1, with an average pool instead of a max pool.
...
class fiftyone.utils.clip.model.LayerNorm(_normalized_shape : int | [list](api__fiftyone.core.session.events.md#fiftyone.core.session.events.Colorscale.list "fiftyone.core.session.events.Colorscale.list")[int] | [Size](https://docs.pytorch.org/docs/stable/size.html#torch.Size "\(in PyTorch v2.12\)")_, _eps : float = 1e-05_, _elementwise_affine : [bool](api__fiftyone.core.stages.md#fiftyone.core.stages.Exists.bool "fiftyone.core.stages.Exists.bool") = True_, _bias : [bool](api__fiftyone.core.stages.md#fiftyone.core.stages.Exists.bool "fiftyone.core.stages.Exists.bool") = True_, _device =None_, _dtype =None_)#
    

Bases: [`LayerNorm`](https://docs.pytorch.org/docs/stable/generated/torch.nn.modules.normalization.LayerNorm.html#torch.nn.modules.normalization.LayerNorm "\(in PyTorch v2.12\)")

Subclass torchâs LayerNorm to handle fp16.
...
elementwise_affine: [bool](api__fiftyone.core.stages.md#fiftyone.core.stages.Exists.bool "fiftyone.core.stages.Exists.bool")#
    

training: [bool](api__fiftyone.core.stages.md#fiftyone.core.stages.Exists.bool "fiftyone.core.stages.Exists.bool")#
    

class fiftyone.utils.clip.model.QuickGELU(_* args: Any_, _** kwargs: Any_)#
...
class fiftyone.utils.clip.model.ResidualAttentionBlock(_d_model : int_, _n_head : int_, _attn_mask : [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor "\(in PyTorch v2.12\)") = None_)#
...
attention(_x : [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor "\(in PyTorch v2.12\)")_)#
...
class fiftyone.utils.clip.model.Transformer(_width : int_, _layers : int_, _heads : int_, _attn_mask : [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor "\(in PyTorch v2.12\)") = None_)#
...
class fiftyone.utils.clip.model.VisionTransformer(_input_resolution : int_, _patch_size : int_, _width : int_, _layers : int_, _heads : int_, _output_dim : int_)#
...
class fiftyone.utils.clip.model.CLIP(_embed_dim : int_, _image_resolution : int_, _vision_layers : Tuple[int, int, int, int] | int_, _vision_width : int_, _vision_patch_size : int_, _context_length : int_, _vocab_size : int_, _transformer_width : int_, _transformer_heads : int_, _transformer_layers : int_)#
...
`encode_image`(image) |   
`encode_text`(text) |   
`forward`(image,Â text) | Define the computation performed at every call.  
`add_module`(name,Â module) | Add a child module to the current module.  
`apply`(fn) | Apply `fn` recursively to every submodule (as returned by `.children()`) as well as self.  
`bfloat16`() | Casts all floating point parameters and buffers to `bfloat16` datatype.  
`buffers`([recurse]) | Return an iterator over module buffers.  
...
encode_image(_image_)#
    

encode_text(_text_)#
    

forward(_image_ , _text_)#
...
fiftyone.utils.clip.model.convert_weights(_model : [Module](https://docs.pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "\(in PyTorch v2.12\)")_)#
    

Converts applicable model parameters to fp16.

fiftyone.utils.clip.model.build_model(_state_dict : dict_)#
