---
source_url: https://docs.voxel51.com/dataset_zoo/datasets_hf/screenspot_pro.html
---
...
[![Hugging Face](https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-Dataset-yellow)](https://huggingface.co/datasets/Voxel51/ScreenSpot-Pro)

# Dataset Card for ScreenSpot-Pro#

![image/png](https://huggingface.co/datasets/Voxel51/ScreenSpot-Pro/resolve/main/screenspot_pro.gif)
...
The dataset consists of 1,581 instruction-image pairs across 23 applications. Each sample includes:
  ...
  1. A high-resolution screenshot (resolutions vary, with the most common being 2560Ã1440 at 32.4% of the data)
...
**Core Fields:**
  ...
  * `application`: EmbeddedDocumentField(Classification) - Application name (e.g., âwordâ)

  * `group`: EmbeddedDocumentField(Classification) - Application category (e.g., âOfficeâ)

  * `platform`: EmbeddedDocumentField(Classification) - Operating system (e.g., âmacosâ)

  * `action_detection`: EmbeddedDocumentField(Detection) - Target interaction element:

    * `label`: Element type (e.g., âtextâ)
...
The data collection prioritized authentic high-resolution screenshots from professional software usage:
  ...
  5. Screens with resolution greater than 1080p (1920Ã1080) were prioritized

  6. Monitor scaling was disabled during capture

  7. For dual-monitor setups, images were captured spanning both displays

  8. UI elements were classified as either âtextâ or âiconâ based on refined criteria
...
The dataset has several limitations:
  ...
  5. The datasetâs focus on high-resolution professional applications may not generalize to other GUI contexts
...
When using this dataset, researchers should:
  ...
  5. Consider developing specialized methods for handling high-resolution inputs, as demonstrated by the authorsâ ScreenSeekeR approach
