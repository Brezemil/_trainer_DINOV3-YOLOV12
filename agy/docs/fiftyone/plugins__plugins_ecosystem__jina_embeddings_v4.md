---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/jina_embeddings_v4.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/jina_embeddings_v4)
...
FiftyOneâs similarity infrastructure requires **fixed-dimension vectors** for storage and search.

### Our Solution: Dual-Mode Support#

#### For Retrieval/Similarity Search (Single-Vector Mode)#

We use Jinaâs native single-vector embeddings:
...
We use Jinaâs multi-vector embeddings with final pooling:
...
**Trade-off** : When using multi-vector mode for embeddings storage, you lose the high dimensionality (2048 â 128) but gain consistency with classification.
...
**Single-Vector Mode (Retrieval/Embeddings)** :
    ...
    Raw multi-vector â Native single-vector compression â Fixed (2048,)
    

**Multi-Vector Mode (Classification)** :
    
    
    Raw multi-vector â Final pooling (mean/max) â Fixed (128,)
...
### Key Implementation Notes#
  ...
  1. **`raw_inputs=True`** : Jinaâs `encode_image()` handles all preprocessing internally
