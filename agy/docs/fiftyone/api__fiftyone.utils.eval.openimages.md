---
source_url: https://docs.voxel51.com/api/fiftyone.utils.eval.openimages.html
---
...
`OpenImagesEvaluationConfig`(pred_field,Â gt_field) | Open Images-style evaluation config.  
---|---  
`OpenImagesEvaluation`(config) | Open Images-style evaluation.  
`OpenImagesDetectionResults`(samples,Â config,Â ...) | Class that stores the results of an Open Images detection evaluation.  
  
class fiftyone.utils.eval.openimages.OpenImagesEvaluationConfig(_pred_field_ , _gt_field_ , _iou =None_, _classwise =None_, _iscrowd ='IsGroupOf'_, _use_masks =False_, _use_boxes =False_, _tolerance =None_, _max_preds =None_, _error_level =1_, _hierarchy =None_, _pos_label_field =None_, _neg_label_field =None_, _expand_gt_hierarchy =True_, _expand_pred_hierarchy =False_, _custom_metrics =None_, _** kwargs_)#
...
Parameters:
  ...
  * **iscrowd** (_"IsGroupOf"_) â the name of the crowd attribute
  ...
  * **max_preds** (_None_) â the maximum number of predicted objects to evaluate when computing mAP and PR curves
...
If `error_level > 0`, any calculation that raises a geometric error will default to an IoU of 0
  ...
  * **hierarchy** (_None_) â an optional dict containing a hierarchy of classes for evaluation following the structure `{"LabelName": label, "Subcategory": [{...}, ...]}`

  * **pos_label_field** (_None_) â the name of a field containing image-level [`fiftyone.core.labels.Classifications`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Classifications "fiftyone.core.labels.Classifications") that specify which classes should be evaluated in the image

  * **neg_label_field** (_None_) â the name of a field containing image-level [`fiftyone.core.labels.Classifications`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Classifications "fiftyone.core.labels.Classifications") that specify which classes should not be evaluated in the image

  * **expand_gt_hierarchy** (_True_) â whether to expand ground truth objects and labels according to the provided `hierarchy`

  * **expand_pred_hierarchy** (_False_) â whether to expand predicted objects and labels according to the provided `hierarchy`
...
class fiftyone.utils.eval.openimages.OpenImagesEvaluation(_config_)#
...
**config** â a `OpenImagesEvaluationConfig`

**Methods:**

`evaluate`(sample_or_frame[,Â eval_key]) | Performs Open Images-style evaluation on the given image.  
---|---  
...
class fiftyone.utils.eval.openimages.OpenImagesDetectionResults(_samples_ , _config_ , _eval_key_ , _matches_ , _precision_ , _recall_ , _classes_ , _thresholds =None_, _missing =None_, _custom_metrics =None_, _backend =None_)#
...
Parameters:
  ...
  * **config** â the `OpenImagesEvaluationConfig` used

  * **eval_key** â the evaluation key

  * **matches** â a list of `(gt_label, pred_label, iou, pred_confidence, gt_id, pred_id)` matches. Either label can be `None` to indicate an unmatched object

  * **precision** â a dict of per-class precision values

  * **recall** â a dict of per-class recall values

  * **classes** â the list of possible classes

  * **thresholds** (_None_) â an optional dict of per-class decision thresholds
  ...
  * **backend** (_None_) â a `OpenImagesEvaluation` backend
...
`plot_pr_curves`([classes,Â num_points,Â backend]) | Plots precision-recall (PR) curves for the detection results.  
---|---  
`mAP`([classes]) | Computes Open Images-style mean average precision (mAP) for the specified classes.  
...
plot_pr_curves(_classes =None_, _num_points =101_, _backend ='plotly'_, _** kwargs_)#
...
Parameters:
  ...
  * **num_points** (_101_) â the number of linearly spaced recall values to plot
...
See [this page](https://storage.googleapis.com/openimages/web/evaluation.html) for more details about Open Images-style mAP.
