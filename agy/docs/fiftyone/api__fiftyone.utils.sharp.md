---
source_url: https://docs.voxel51.com/api/fiftyone.utils.sharp.html
---

# fiftyone.utils.sharp#

[Apple SHARP](https://github.com/apple/ml-sharp) wrapper for the FiftyOne Model Zoo.
...
`AppleSharpModelConfig`(d) | Configuration for running a `AppleSharpModel`.  
---|---  
`AppleSharpModel`(config) | Wrapper for running [Apple SHARP](https://github.com/apple/ml-sharp) inference.  
  
class fiftyone.utils.sharp.AppleSharpModelConfig(_d_)#
...
Configuration for running a `AppleSharpModel`.
...
Parameters:
  ...
  * **focal_length_mm** (_26.0_) â the focal length in mm (35mm equivalent). Used to compute disparity factor. Most smartphone photos use 26-28mm

  * **output_dir** (_None_) â directory to save output .ply files. If None, uses a temporary directory
...
class fiftyone.utils.sharp.AppleSharpModel(_config_)#
...
Wrapper for running [Apple SHARP](https://github.com/apple/ml-sharp) inference.

SHARP (SHarp monoculAR sPlatting) generates photorealistic 3D Gaussian representations from a single image in under one second.
...
**config** â a `AppleSharpModelConfig`
...
`focal_length_to_fpx`(width) | Convert 35mm-equivalent focal length in mm to focal length in px.  
---|---  
`disparity_factor`(width) | Compute the SHARP disparity factor for an image width.  
`compute_intrinsics`(height,Â width,Â f_px) | Build the 4x4 intrinsics matrix for the input image.  
`resize_intrinsics`(intrinsics,Â ...) | Scale intrinsics for the resized SHARP inference shape.  
