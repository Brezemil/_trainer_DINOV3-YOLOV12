---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/zero_shot_prediction.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/jacobmarks/zero-shot-prediction-plugin)
...
_Note_ â For SAM you will need to have Facebookâs `segment-anything` library installed.
...
These models are âregisteredâ via dictionaries in each file. In `semantic_segmentation.py`, for example, the dictionary is:
...
All of the operators in this plugin can be run in _delegated_ execution mode. This means that instead of waiting for the operator to finish, you _schedule_ the operation to be performed separately. This is useful for long-running operations, such as performing inference on a large dataset.

Once you have pressed the `Schedule` button for the operator, you will be able to see the job from the command line using FiftyOneâs [command line interface](https://docs.voxel51.com/cli/index.html#fiftyone-delegated-operations):
