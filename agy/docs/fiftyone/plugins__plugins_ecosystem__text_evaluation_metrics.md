---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/text_evaluation_metrics.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/text_evaluation_metrics)

# Text Evaluation Metrics for FiftyOne#

![image](https://raw.githubusercontent.com/harpreetsahota204/text_evaluation_metrics/main/text_eval_gif.gif)

Operator plugin for evaluating text fields (StringFields) in FiftyOne datasets with standard VLM OCR metrics â including surface-form metrics, structural metrics for JSON/HTML/Markdown output, and semantic similarity.
...
---|---|---|---  
**ANLS** | Average Normalized Levenshtein Similarity with threshold | Primary OCR metric for VLMs, robust to minor errors | 0.0â1.0  
**Exact Match** | Binary perfect match | Strict evaluation (form fields, IDs) | 0.0 or 1.0  
**Normalized Similarity** | Continuous Levenshtein similarity without threshold | Fine-grained analysis and ranking | 0.0â1.0  
...
**TED Similarity** | Tree Edit Distance similarity for structured text | Evaluating JSON, HTML, or Markdown output from VLMs | 0.0â1.0  
**Semantic Similarity** | Cosine similarity of sentence-transformer embeddings | Meaning-level evaluation, paraphrase-tolerant scoring | 0.0â1.0  
...
### Choosing the right metric#
  ...
  * **Multi-metric analysis** : run several metrics and correlate results to understand _where_ and _how_ a model is failing.
...
**Binary exact match accuracy** \- Returns 1.0 only for perfect matches, ideal for strict evaluation where partial credit isnât appropriate
...
This is the appropriate metric when a VLM is expected to return structured output. Unlike character-level metrics, TED captures whether the _structure_ is correct â a prediction with the right text but wrong nesting scores lower than a prediction with a minor typo in an otherwise correct structure.
...
**Format notes:**
  ...
  * `"json"` â objects are sorted by key before comparison (JSON is semantically unordered). Markdown code fences (````json ````) are stripped automatically.

  * `"html"` â tag names are used as node labels; text content becomes leaf nodes; whitespace-only nodes are ignored.

  * `"markdown"` â the AST node type (e.g. `Heading`, `Paragraph`, `List`) is used as the label; leaf text content is the leaf label.
...
**Semantic Similarity** \- Embeds both strings using a pre-trained sentence encoder and computes their cosine similarity. Captures _meaning_ rather than surface form, so paraphrases and synonymous expressions score near 1.0 even without lexical overlap.
...
---|---|---|---  
`all-MiniLM-L6-v2` |  |  | Default â best speed/quality balance  
...
### Via FiftyOne App#
  ...
  3. Search for the metric you want to compute (e.g., âCompute ANLSâ)
...
## Best Practices#
  ...
  1. **Start with ANLS for plain OCR tasks** : Itâs the standard metric for VLM OCR and is robust to minor transcription errors.
  ...
  7. **Evaluate on views** : Use FiftyOneâs filtering to evaluate metric performance on specific subsets (e.g., only samples with a particular label, or samples where ANLS is below a threshold).
...
### Evaluating Structured JSON Output#
    ...
    # (possibly wrapped in ```json ... ``` fences â these are stripped automatically)
...
### Semantic Similarity for Open-Ended Answers#
    ...
    # Default model (all-MiniLM-L6-v2) â fast, great for most use cases
    result = sem_op(
        dataset,
