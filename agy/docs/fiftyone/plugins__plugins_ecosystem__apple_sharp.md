---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/apple_sharp.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/apple_sharp)

# Apple SHARP - FiftyOne Model Zoo Integration#

![sharp](https://cdn.voxel51.com/plugins/apple_sharp/sharp.gif)

[SHARP](https://github.com/apple/ml-sharp) is Appleâs state-of-the-art model for predicting 3D Gaussian Splats from a single RGB image. This integration brings SHARP to FiftyOne, enabling batch inference on image datasets with 3D visualization.
...
While the SHARP neural network itself runs inference very fast (~1 second per image on GPU), the overall pipeline includes additional operations that add some overhead:
  ...
  1. **Image Loading** : SHARPâs `load_rgb` reads EXIF data for focal length extraction
...
These read/write operations are necessary for compatibility with standard PLY viewers and FiftyOneâs 3D visualization.

* * *

## Technical Details: Converting 3DGS PLY to Standard PLY#

Appleâs SHARP outputs 3D Gaussian Splat (3DGS) files in PLY format with specialized properties that standard PLY viewers donât understand. This integration converts them to viewer-compatible PLY files with standard RGB colors.
...
To convert SH to RGB:
    ...
    RGB = 0.5 + SH_C0 Ã f_dc
    

Where `SH_C0 = 0.28209...` is the zero-order SH basis function constant (`1 / (2âÏ)`). The 0.5 offset centers the color range since SH coefficients are centered around zero.
