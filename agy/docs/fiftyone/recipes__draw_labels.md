---
source_url: https://docs.voxel51.com/recipes/draw_labels.html
---

|  [ ![](https://cdn.voxel51.com/colab-logo-256px.png) Run in Google Colab ](https://colab.research.google.com/github/voxel51/fiftyone/blob/main/docs/source/recipes/draw_labels.ipynb) |  [ ![](https://cdn.voxel51.com/github-logo-256px.png) View source on GitHub ](https://github.com/voxel51/fiftyone/blob/main/docs/source/recipes/draw_labels.ipynb) |  [ ![](https://cdn.voxel51.com/cloud-icon-256px.png) Download notebook ](https://raw.githubusercontent.com/voxel51/fiftyone/main/docs/source/recipes/draw_labels.ipynb)
...
In this recipe weâll use the [FiftyOne Dataset Zoo](https://docs.voxel51.com/dataset_zoo/index.html) to download some labeled datasets to use as sample data for drawing labels. Behind the scenes, FiftyOne uses either the [TensorFlow Datasets](https://www.tensorflow.org/datasets) or [TorchVision Datasets](https://pytorch.org/vision/stable/datasets.html) libraries to wrangle the datasets, depending on which ML library you have installed. You can, for example, install PyTorch as follows (weâll also need `pycocotools` to load the COCO dataset, in particular):
...
Now letâs load the dataset, extract a [DatasetView](https://voxel51.com/docs/fiftyone/user_guide/using_datasets.html#datasetviews) that contains 100 images from the dataset, and render them as annotated images with their ground truth labels overlaid:
    ...
    Loading 'coco-2017' split 'validation'
     100% |âââââ| 5000/5000 [14.8s elapsed, 0s remaining, 339.4 samples/s]
    Writing annotated images to '/tmp/fiftyone/draw_labels/coco-2017-validation-anno'
     100% |âââââââ| 100/100 [7.3s elapsed, 0s remaining, 11.9 samples/s]
    Annotation complete
    

Letâs list the output directory to verify that the annotations have been generated:
...
Hereâs an example of an annotated image that was generated: ![coco-2017-annotated](../_images/draw_labels_coco2017.jpg)
...
Now letâs load the dataset, extract a [DatasetView](https://voxel51.com/docs/fiftyone/user_guide/using_datasets.html#datasetviews) that contains 100 images from the dataset, and render them as annotated images with their ground truth labels overlaid:
    ...
    Loading 'caltech101' split 'test'
     100% |âââââ| 9145/9145 [4.8s elapsed, 0s remaining, 1.9K samples/s]
    Writing annotated images to '/tmp/fiftyone/draw_labels/caltech101-test-anno'
     100% |âââââââ| 100/100 [2.6s elapsed, 0s remaining, 37.4 samples/s]
    Annotation complete
...
Hereâs an example of an annotated image that was generated: ![61faa5e4d5504c6bb4f9b53b9271d56e](../_images/draw_labels_caltech101.jpg)

## Drawing labels on videos#

FiftyOne can also render frame labels onto video samples. To demonstrate, letâs work with the (small) video quickstart dataset from the zoo:
    ...
    Loading 'quickstart-video'
     100% |âââââââââ| 10/10 [15.9s elapsed, 0s remaining, 0.6 samples/s]
...
Note that the dataset contains frame-level detections in the `objs` field of each frame. Letâs make a [DatasetView](https://voxel51.com/docs/fiftyone/user_guide/using_datasets.html#datasetviews) that contains a couple random videos from the dataset and render them as annotated videos with the frame-level detections overlaid:
    ...
    Rendering video 1/2: '/tmp/fiftyone/draw_labels/quickstart-video-anno/0587e1cfc2344523922652d8b227fba4-000014-video_052.mp4'
     100% |ââââââââ| 120/120 [19.0s elapsed, 0s remaining, 6.7 frames/s]
    Rendering video 2/2: '/tmp/fiftyone/draw_labels/quickstart-video-anno/0587e1cfc2344523922652d8b227fba4-000014-video_164.mp4'
     100% |ââââââââ| 120/120 [27.2s elapsed, 0s remaining, 4.3 frames/s]
    Annotation complete
...
Hereâs a snippet of an annotated video that was generated: ![quickstart-video-annotated](../_images/draw_labels_quickstart_video.gif)
