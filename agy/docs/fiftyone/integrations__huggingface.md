---
source_url: https://docs.voxel51.com/integrations/huggingface.html
---
...
You can pass `transformers` classification models directly to FiftyOne datasetâs [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model") method:
...
Alternatively, you can manually run inference with the `transformers` model and then use the `to_classification()` utility to convert the predictions to [FiftyOne format](https://docs.voxel51.com/user_guide/using_datasets.html#classification):
...
Finally, you can load `transformers` models directly from the [FiftyOne Model Zoo](https://docs.voxel51.com/model_zoo/index.html#model-zoo)!
...
You can pass `transformers` detection models directly to your FiftyOne datasetâs [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model") method:
...
Alternatively, you can manually run inference with the `transformers` model and then use the `to_detections()` utility to convert the predictions to [FiftyOne format](https://docs.voxel51.com/user_guide/using_datasets.html#object-detection):
...
You can pass a `transformers` semantic segmentation model directly to your FiftyOne datasetâs [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model") method:
...
Alternatively, you can manually run inference with the `transformers` model and then use the `to_segmentation()` utility to convert the predictions to [FiftyOne format](https://docs.voxel51.com/user_guide/using_datasets.html#semantic-segmentation):
...
You can pass a `transformers` monocular depth estimation model directly to your FiftyOne datasetâs [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model") method:
...
Alternatively, you can load `transformers` depth estimation models directly from the [FiftyOne Model Zoo](https://docs.voxel51.com/model_zoo/index.html#model-zoo)!
...
Zero-shot image classification models from `transformers` can be loaded directly from the [FiftyOne Model Zoo](https://docs.voxel51.com/model_zoo/index.html#model-zoo)!
...
Once loaded, you can pass the model directly to your FiftyOne datasetâs [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model") method:
...
Zero-shot object detection models from `transformers` can be loaded directly from the [FiftyOne Model Zoo](https://docs.voxel51.com/model_zoo/index.html#model-zoo)!
...
When using [`apply_model()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.apply_model "fiftyone.core.collections.SampleCollection.apply_model"), you can request batch inference by passing the optional `batch_size` parameter:
...
See [this section](https://docs.voxel51.com/user_guide/using_datasets.html#batch-updates) for more information about performing batch updates to your FiftyOne datasets.
...
To compute embeddings for images, you can pass the `transformers` model directly to your FiftyOne datasetâs [`compute_embeddings()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.compute_embeddings "fiftyone.core.collections.SampleCollection.compute_embeddings") method:
...
Alternatively, you can use the `convert_transformers_model()` utility to convert a `transformers` model to FiftyOne format, which allows you to check the modelâs [`has_embeddings`](api__fiftyone.core.models.md#fiftyone.core.models.Model.has_embeddings "fiftyone.core.models.Model.has_embeddings") property to see if the model can be used to generate embeddings:
...
You can request batch inference by passing the optional `batch_size` parameter to [`compute_embeddings()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.compute_embeddings "fiftyone.core.collections.SampleCollection.compute_embeddings"):
...
You can compute embeddings for image patches by passing `transformers` models directly to your FiftyOne datasetâs [`compute_patch_embeddings()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.compute_patch_embeddings "fiftyone.core.collections.SampleCollection.compute_patch_embeddings") method:
...
To use this functionality, you must pass the model by **name** into the brain method, along with any necessary keyword arguments that must be passed to [`load_zoo_model()`](api__fiftyone.zoo.md#fiftyone.zoo.load_zoo_model "fiftyone.zoo.load_zoo_model") to load the correct model:
...
If you are working with a dataset in FiftyOne and you want to quickly share it with others, you can do so via the [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub") function, which takes two positional arguments:

  * the FiftyOne sample collection (a [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") or [`DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView"))
...
The basic recipe for pushing a FiftyOne dataset to the Hub is just two lines of code. As a starting point, letâs use the example [Quickstart dataset](dataset_zoo__datasets__quickstart.md#dataset-zoo-quickstart) dataset from the [FiftyOne Dataset Zoo](https://docs.voxel51.com/dataset_zoo/index.html#dataset-zoo):
...
To push the dataset to the Hugging Face Hub, all you need to do is call [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub") with the dataset and the desired `repo_name`:
...
When you run this code, a few things happen:
  ...
  * A `fiftyone.yml` config file for the dataset is generated and uploaded to the repo, which contains all of the necessary information so that the dataset can be loaded with [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub").
...
Pushing a [`DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") to the Hub works in exactly the same way. For example, if you want to push a filtered view of the `quickstart` dataset containing only predictions with high confidence, you can do so by creating the view as usual, and then passing that in to [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub"):
...
Large datasets with many samples require a bit more care when pushing to the Hub. Hugging Face limits the number of files that can be uploaded in a single directory to 10000, so if your dataset contains more than 10000 samples, the data will need to be split into multiple directories. FiftyOne handles this automatically when pushing large datasets to the Hub, but you can manually configure the number of samples per directory by passing the `chunk_size` argument to [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub"):
...
The `chunk_size` argument is currently only supported when exporting in [FiftyOneDataset format](user_guide__export_datasets.md#fiftyonedataset-export) (the default).

#### Advanced usage#

The [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub") function provides a number of optional arguments that allow you to customize how your dataset is pushed to the Hub, including whether the dataset is public or private, what license it is released under, and more.

FiftyOneâs [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub") function supports the Hugging Face Hub API arguments `private` and `exist_ok`.
...
The `tags` argument can be a string or a list of strings. The tag `fiftyone` is automatically added to all datasets pushed with FiftyOne, communicating that the dataset was created with FiftyOne and can be loaded with the [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function.
...
The `description` argument can be used for whatever you like. When the dataset is loaded from the Hub, this description will be accessible via the datasetâs [`description`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.description "fiftyone.core.dataset.Dataset.description") property.

Additionally, you can specify the âformatâ of the uploaded dataset. By default, the format is the standard [FiftyOneDataset](user_guide__import_datasets.md#fiftyonedataset-import) format, but you can also specify the data is uploaded in any of these [common formats](user_guide__import_datasets.md#supported-import-formats). For example, to push the quickstart dataset in [COCO](user_guide__import_datasets.md#cocodetectiondataset-import) format, with a Creative Commons Attribution 4.0 license, you can do the following:
...
To load a dataset from the Hugging Face Hub, you can use the [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function.
...
This function supports loading datasets in any of the [common formats](user_guide__import_datasets.md#supported-import-formats) supported by FiftyOne, as well as image-based datasets stored via [Parquet](https://parquet.apache.org/) files, as is common with datasets from the [datasets](https://huggingface.co/docs/datasets/en/index) library which have been uploaded to the Hugging Face Hub. Below, we will walk through all of the ways you can load datasets from the Hub.

In its simplest usage, the [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function only requires the `repo_id` of the dataset you want to load. For example, to load the private dataset that we pushed to the Hub earlier, you can do the following:
...
When you push a dataset to the Hub using [`push_to_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.push_to_hub "fiftyone.utils.huggingface.push_to_hub"), a `fiftyone.yml` config file is generated and uploaded to the repo. This file contains all of the information necessary to load the dataset from the Hugging Face Hub. More generally, any repo on the Hugging Face Hub that contains a `fiftyone.yml` or `fiftyone.yaml` file (assuming the file is correctly formatted) can be loaded using the [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function by passing the `repo_id` of the dataset, without needing to specify any additional arguments.
...
In addition to loading datasets from repo configs and local configs, you can also load datasets from the Hub by passing the necessary config arguments directly to [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub"). This is useful when you want to load a dataset from the Hub that does not have a `fiftyone.yml` or `fiftyone.yaml` file, and the structure of the dataset is simple enough that you can specify the necessary arguments directly.
...
This will tell FiftyOne that the data is stored in Parquet files, and that the `label` field should be treated as a classification field, to be converted into a [`Classification`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Classification "fiftyone.core.labels.Classification") label field in the dataset.
...
**Format specification** :
  ...
  * **format** _(str)_ : The format of the dataset. This can be any of the [common formats](user_guide__import_datasets.md#supported-import-formats) supported by FiftyOne â just pass the name of the format as a string. For example, to load a dataset in the [COCO](user_guide__import_datasets.md#cocodetectiondataset-import) format, you can pass `format="COCODetectionDataset"`. To specify that the dataset is stored in Parquet files, you can pass `format="ParquetFilesDataset"` (or simply `format="parquet"` for short). This is the only required field.

  * **name** _(str)_ : The name of the FiftyOne [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") to be created. If the `repo_id` is cumbersome, this can be used to specify a simpler default name. For example, for this [sheep dataset](https://huggingface.co/datasets/keremberke/aerial-sheep-object-detection) rather than using the `repo_id` `keremberke/aerial-sheep-object-detection`, you can specify `name="sheep-detection"`.
...
FiftyOneâs [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function supports a variety of arguments that allow you to control the download process, from the maximum number of samples to be downloaded to the batch size to use when making requests to the Datasets Server. Here are the supported arguments:
  ...
  * **num_workers** _(int)_ : The number of worker to use when downloading media files. If not specified, the number of workers will be resolved by looking at your [FiftyOne Config](user_guide__config.md#configuring-fiftyone).
...
Okay, so [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") is _very_ powerful, and can be used in a _ton_ of ways. All of this flexibility can be a bit overwhelming, so letâs walk through a few examples to show you how easy it is in practice to load datasets from the Hugging Face Hub.
...
Letâs start by loading the [MNIST](https://huggingface.co/datasets/ylecun/mnist) dataset into FiftyOne. All you need to do is pass the `repo_id` of the dataset âÂ in this case `"ylecun/mnist"` â to [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub"), specify the format as `"parquet"`, and specify the `classification_fields` as `"label"`:
...
Once you have the dataset loaded into FiftyOne, you may want to set the datasetâs [mask targets](https://docs.voxel51.com/integrations/storing-mask-targets) to specify the names of the classes represented in the segmentation masks.
...
The [`load_from_hub()`](api__fiftyone.utils.huggingface.md#fiftyone.utils.huggingface.load_from_hub "fiftyone.utils.huggingface.load_from_hub") function also allows us to load datasets in much more complex formats, as well as with more advanced configurations. Letâs walk through a few examples to show you how to leverage the full power of FiftyOneâs Hugging Face Hub integration.
