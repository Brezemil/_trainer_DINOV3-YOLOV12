---
source_url: https://docs.voxel51.com/api/fiftyone.core.evaluation.html
---
...
`EvaluationInfo`(key[,Â version,Â timestamp,Â config]) | Information about an evaluation that has been run on a dataset.  
---|---  
`EvaluationMethodConfig`(**kwargs) | Base class for configuring `EvaluationMethod` instances.  
`EvaluationMethod`(config) | Base class for evaluation methods.  
`EvaluationResults`(samples,Â config,Â key[,Â ...]) | Base class for evaluation results.  
  
class fiftyone.core.evaluation.EvaluationInfo(_key_ , _version =None_, _timestamp =None_, _config =None_)#
...
Parameters:
  ...
  * **key** â the evaluation key

  * **timestamp** (_None_) â the UTC `datetime` when the evaluation was run

  * **config** (_None_) â the `EvaluationMethodConfig` for the evaluation
...
class fiftyone.core.evaluation.EvaluationMethodConfig(_** kwargs_)#
    

Bases: [`BaseRunConfig`](api__fiftyone.core.runs.md#fiftyone.core.runs.BaseRunConfig "fiftyone.core.runs.BaseRunConfig")

Base class for configuring `EvaluationMethod` instances.
...
class fiftyone.core.evaluation.EvaluationMethod(_config_)#
...
Subclasses will typically declare an interface method that handles performing evaluation on an image, video, or entire collection.

Parameters:
    

**config** â an `EvaluationMethodConfig`
...
class fiftyone.core.evaluation.EvaluationResults(_samples_ , _config_ , _key_ , _backend =None_)#
