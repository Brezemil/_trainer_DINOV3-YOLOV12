...
A plugin that provides operators for extracting structured data from visual sources using [VLM Run](https://vlm.run)âs vision-language models.
...
Flexible media analysis and content generation using VLM Runâs Orion chat completions API with natural language prompts.

**How to use:**

  1. Press ``` or click `Browse operations` to open the operator list

  2. Search for âChat Completionsâ and select it
...
The operator automatically detects your datasetâs media type (image, video, or document) and filters the available modes and toolsets to only show compatible options.

**Modes:**

  * **Analyze** â Describe, caption, or extract information from your media using natural language prompts. Results are stored as text in a sample field, along with model and usage metadata fields. Works with images, videos, and PDFs.

  * **Annotate** â Produce spatial annotations: bounding boxes (Detections), keypoints, or segmentation masks. Results are stored as native FiftyOne label types. Available for image and document datasets (not video).

  * **Edit** â Modify existing media with natural language instructions (e.g., âBlur all facesâ, âTrim to the first 5 secondsâ). Edited files are saved and added to the dataset with the `vlmrun_edited` tag.

  * **Generate** â Create new images or videos from text prompts (no input media required). Supports generating multiple outputs at once. Generated files are saved and added to the dataset with the `vlmrun_generated` tag.
...
This operator uses VLM Runâs object detection domain with visual grounding to extract precise bounding boxes in normalized xywh format:
...
This operator uses VLM Runâs person detection domain optimized for detecting people in various scenarios:
...
This operator uses VLM Runâs invoice parsing domain:
...
This operator uses VLM Runâs layout detection capabilities:
...
This operator provides comprehensive video analysis using VLM Runâs video understanding capabilities:
