---
source_url: https://docs.voxel51.com/recipes/merge_datasets.html
---

[ ![](https://cdn.voxel51.com/colab-logo-256px.png) Run in Google Colab ](https://colab.research.google.com/github/voxel51/fiftyone/blob/main/docs/source/recipes/merge_datasets.ipynb) |  [ ![](https://cdn.voxel51.com/github-logo-256px.png) View source on GitHub ](https://github.com/voxel51/fiftyone/blob/main/docs/source/recipes/merge_datasets.ipynb) |  [ ![](https://cdn.voxel51.com/cloud-icon-256px.png) Download notebook ](https://raw.githubusercontent.com/voxel51/fiftyone/main/docs/source/recipes/merge_datasets.ipynb)

# Merging Datasets#

This recipe demonstrates a simple pattern for merging FiftyOne Datasets via [Dataset.merge_samples()](https://voxel51.com/docs/fiftyone/api/fiftyone.core.dataset.html?highlight=merge_samples#fiftyone.core.dataset.Dataset.merge_samples). Merging datasets is an easy way to:
...
In this recipe, weâll work with a dataset downloaded from the [FiftyOne Dataset Zoo](https://docs.voxel51.com/dataset_zoo/index.html). To access the dataset, install `torch` and `torchvision`, if necessary:
...
Load the test split of CIFAR-10 into FiftyOne:
    ...
    Loading 'cifar10' split 'test'
     100% |âââ| 10000/10000 [14.1s elapsed, 0s remaining, 718.2 samples/s]
    Name:           merge-example
    Media type:     image
    Num samples:    10000
    Persistent:     False
...
Suppose you would like to add model predictions to some samples from the dataset. The usual way to do this is to just iterate over the dataset and add your predictions directly to the samples:
...
You can easily merge the `predictions` dataset into the main dataset via [Dataset.merge_samples()](https://voxel51.com/docs/fiftyone/api/fiftyone.core.dataset.html?highlight=merge_samples#fiftyone.core.dataset.Dataset.merge_samples). Letâs start by creating a fresh copy of CIFAR-10 that doesnât have predictions:
...
Now letâs merge the predictions into the fresh dataset:
...
Letâs print a sample with predictions to verify that the merge happened as expected:
...
By default, samples with the same absolute `filepath` are merged. However, you can customize this as desired via various keyword arguments of [Dataset.merge_samples()](https://voxel51.com/docs/fiftyone/api/fiftyone.core.dataset.html?highlight=merge_samples#fiftyone.core.dataset.Dataset.merge_samples). For example, the command below will merge samples with the same base filename, ignoring the directory:
    ...
    Indexing dataset...
     100% |âââ| 10000/10000 [3.6s elapsed, 0s remaining, 2.8K samples/s]
    Merging samples...
     100% |âââââââ| 100/100 [348.5ms elapsed, 0s remaining, 287.0 samples/s]
