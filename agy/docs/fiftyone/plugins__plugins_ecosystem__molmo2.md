---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/molmo2.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/molmo2)

# Molmo2 - FiftyOne Model Zoo Integration#

![](https://raw.githubusercontent.com/harpreetsahota204/molmo2/main/molmo2.gif)
...
Molmo2âs core strength is **grounding** â precise pixel-level localization and tracking over time. While standard models might say âa car passed by,â Molmo2 can point to the **exact car** , follow it **through a crowd** , and mark the **exact second** it crossed a line.

Domain | Applications  
---|---  
**Robotics & Automation** | Spatial affordance prediction, action counting (âHow many times did the robot grasp the block?â), event localization  
...
**describe:**
  ...
  * Sample-level string field containing the modelâs text response
...
**Added field:** `molmo_embeddings` â a numpy array of shape `(hidden_dim,)` on each sample.

**Pooling strategy:** Controls how variable-length hidden states are collapsed into a fixed-size vector:

  * `mean` (default) â average across all tokens

  * `max` â max pooling across tokens

  * `cls` â use the first (CLS) token only
...
**Added field:** A string field (e.g., `prompted_describe` or `answer_pred`) containing the modelâs text response on each sample.
...
**Added fields:** Multiple fields are added to each sample:
  ...
  * `comprehensive_summary` â text description

  * `comprehensive_events` â `fo.TemporalDetections` for activities/events

  * `comprehensive_objects` â `fo.TemporalDetections` with first/last appearance times

  * `comprehensive_scene_info_*` â `fo.Classification` fields (setting, time_of_day, location_type)

  * `comprehensive_activities_*` â `fo.Classification` or `fo.Classifications`
