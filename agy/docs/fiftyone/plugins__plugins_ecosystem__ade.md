---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/ade.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/landing-ai/ade-51)

# LandingAI ADE Plugin#

[ai_landing.webm](https://github.com/user-attachments/assets/9b17a479-0e8e-4b0a-a21c-733c45cf35ac)

A FiftyOne plugin that provides operators for parsing, extracting, and splitting documents using [LandingAI](https://landing.ai)âs Agentic Document Extraction (ADE) API. Converts PDFs, images, spreadsheets, and Office files into structured Markdown with spatial bounding box grounding stored as native FiftyOne `Detections`.
...
Set your LandingAI API key as the `VISION_AGENT_API_KEY` environment variable or add it to [FiftyOne secrets](https://docs.voxel51.com/plugins/using_plugins.html#secrets). You can obtain an API key from the [LandingAI dashboard](https://va.landing.ai).
...
Define each field with a name, description, and type (string, number, or boolean). Values are stored as flat, properly-typed top-level sample fields so FiftyOne shows the right filter widget in the App sidebar â range slider for numbers, toggle for booleans, text search for strings.
...
`ade_splits_type` | `Classification` | Primary document type (first splitâs classification)  
