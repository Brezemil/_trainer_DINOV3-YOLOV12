...
You can bring up the operators by pressing â`â and searching for their name. Below you can find the available operators.
...
Current limitations:
  ...
  * You canât create a `multisensor-sequence` Segments.ai dataset using this operator. Instead, you can create a dataset using the segments.ai web interface and use the plugin to setup your annotation task.
...
Current limitations:
  ...
  * Itâs currently not possible to fetch annotations for Segments.ai sequences (except `multisensor-sequences`).
...
In case you are using a non-sequence Segments.ai dataset, the matching mechanism simply looks up the corresponding fiftyone sample based on the segments sample UUID. These are automatically stored in the fiftyone sample object as an attribute, under the key âsegments_uuidâ. If you have uploaded your dataset to Segments.ai using the `request_annotations` operator, this is done for you automatically! If not, you will have to provide this information yourself. This can be done as follows:
...
Alternatively, this example uses FiftyOneâs [set_values()](https://docs.voxel51.com/api/fiftyone.core.dataset.html?highlight=set_values#fiftyone.core.dataset.Dataset.set_values) method to perform a bulk update:
...
Sample.metadata.camera_convention | The camera convention used for the calibration. Should be either âOpenGLâ or âOpenCVâ  
