---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/albumentations_augmentation.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/jacobmarks/fiftyone-albumentations-plugin)
...
Traditionally, data augmentation is performed on-the-fly during training. This is greatâ¦ _if_ you know exactly what augmentations you want to apply to your dataset.

However, if youâre just getting started with a new dataset, you may not know what augmentations are appropriate for your data. In this case, it can be helpful to apply a wide range of augmentations to your dataset and then manually inspect the results to see which augmentations are appropriate for your data.
...
To get started, launch the [FiftyOne App](https://docs.voxel51.com/user_guide/app.html) and load in your dataset. If you donât have a dataset yet, you can use the [FiftyOne Dataset Zoo](https://docs.voxel51.com/user_guide/dataset_zoo/index.html) to load in a sample dataset.
...
By default, each time a new augmentation is applied to your dataset, all of the samples that were created by the _previous_ augmentation are removed from your dataset, and the temporary files that were created to store the samples are deleted.
