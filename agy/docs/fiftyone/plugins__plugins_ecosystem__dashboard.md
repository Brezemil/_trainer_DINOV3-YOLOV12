---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/dashboard.html
---

[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/voxel51/fiftyone-plugins/tree/main/plugins/dashboard)

# Dashboard Plugin#

A plugin that enables users to construct custom dashboards that display statistics of interest about the current dataset (and beyond).
...
The Dashboard panel supports defining arbitrary custom plots via Python code. This feature is useful for defining plots that require custom aggregations or for more advanced use cases such as defining plots that operate across multiple datasets.
...
All data sources must define `data` object that describes a valid [plotly-react](https://plotly.com/javascript/react) plot for the chart type that youâve selected in the dashboardâs plot creation modal:
...
> Do **not** set the `type` property of the `data` object; it will be automatically populated based on the chart type youâve selected in the modal

Your code has access to a `ctx` variable that contains the dashboardâs [ExecutionContext](https://docs.voxel51.com/plugins/developing_plugins.html#execution-context):
...
The following custom data source defines a simple pie chart of label counts in a specified field of the current dataset:
...
The following custom data source defines a histogram that aggregates values across multiple datasets:
