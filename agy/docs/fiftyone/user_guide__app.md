---
source_url: https://docs.voxel51.com/user_guide/app.html
---

# Using the FiftyOne App#

The FiftyOne App is a powerful graphical user interface that enables you to visualize, browse, and interact directly with your [datasets](https://docs.voxel51.com/user_guide/using_datasets.html#using-datasets).

![app-filters](../_images/app-filters.gif)

Note

Did you know? You can use FiftyOneâs [plugin framework](plugins__overview.md#fiftyone-plugins) to customize and extend the behavior of the App!

## App environments#

The FiftyOne App can be used in any environment that youâre working in, from a local IPython shell, to a remote machine or cloud instance, to a Jupyter or Colab notebook. Check out the [environments guide](installation__environments.md#environments) for best practices when working in each environment.

## Sessions#

The basic FiftyOne workflow is to open a Python shell and load a [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset"). From there you can launch the FiftyOne App and interact with it programmatically via a _session_.

### Creating a session#

You can launch an instance of the App by calling [`launch_app()`](api__fiftyone.core.session.md#fiftyone.core.session.launch_app "fiftyone.core.session.launch_app"). This method returns a [`Session`](api__fiftyone.core.session.md#fiftyone.core.session.Session "fiftyone.core.session.Session") instance, which you can subsequently use to interact programmatically with the App!
...
App sessions are highly flexible. For example, you can launch [launch multiple App instances](https://docs.voxel51.com/faq/index.html#faq-multiple-apps) and connect multiple App instances to the [same dataset](https://docs.voxel51.com/faq/index.html#faq-multiple-sessions-same-dataset).

By default, when youâre working in a non-notebook context, the App will be opened in a new tab of your web browser. See [this FAQ](https://docs.voxel51.com/faq/index.html#faq-supported-browsers) for supported browsers.

Note

[`fo.launch_app()`](api__fiftyone.core.session.md#fiftyone.core.session.launch_app "fiftyone.core.session.launch_app") will launch the App asynchronously and return control to your Python process. The App will then remain connected until the process exits.

Therefore, if you are using the App in a script, you should use [`session.wait()`](api__fiftyone.core.session.md#fiftyone.core.session.Session.wait "fiftyone.core.session.Session.wait") to block execution until you close it manually:
...
When working inside a Docker container, FiftyOne should automatically detect and appropriately configure networking. However, if you are unable to load the App in your browser, you many need to manually [set the App address](installation__environments.md#restricting-app-address) to `0.0.0.0`:
    
    
    session = fo.launch_app(..., address="0.0.0.0")
    

See [this page](installation__environments.md#docker) for more information about working with FiftyOne inside Docker.
...
Sessions can be updated to show a new [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") by updating the [`Session.dataset`](api__fiftyone.core.session.md#fiftyone.core.session.Session.dataset "fiftyone.core.session.Session.dataset") property of the session object:
...
You can also show a specific [view](https://docs.voxel51.com/user_guide/using_views.html#using-views) into the current dataset in the App by setting the [`Session.view`](api__fiftyone.core.session.md#fiftyone.core.session.Session.view "fiftyone.core.session.Session.view") property of the session.

For example, the command below loads a [`DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") in the App that shows the first 10 samples in the dataset sorted by their `uniqueness` field:
...
You can immediately load a specific sample in the modal when launching a new [`Session`](api__fiftyone.core.session.md#fiftyone.core.session.Session "fiftyone.core.session.Session") by providing its ID via the `sample_id` parameter:
...
You can also programmatically load a sample in the modal on an existing session by setting its [`session.sample_id`](api__fiftyone.core.session.md#fiftyone.core.session.Session.sample_id "fiftyone.core.session.Session.sample_id") property:
...
Similarly, for [group datasets](https://docs.voxel51.com/user_guide/groups.html#groups), you can immediately load a specific group in the modal when launching a new [`Session`](api__fiftyone.core.session.md#fiftyone.core.session.Session "fiftyone.core.session.Session") by providing its ID via the `group_id` parameter:
...
You can also programmatically load a group in the modal on an existing session by setting its [`session.group_id`](api__fiftyone.core.session.md#fiftyone.core.session.Session.group_id "fiftyone.core.session.Session.group_id") property:
...
Check out the [environments page](installation__environments.md#environments) for more information on possible configurations of local/remote/cloud data and App access.

### Remote machine#

On the remote machine, you can load a [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") and launch a remote session using either the Python library or the CLI.

Load a [`Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") and call [`launch_app()`](api__fiftyone.core.session.md#fiftyone.core.session.launch_app "fiftyone.core.session.launch_app") with the `remote=True` argument.
...
You can use the optional `port` parameter to choose the port of your remote machine on which to serve the App. The default is `5151`, which can also be customized via the `default_app_port` parameter of your [FiftyOne config](user_guide__config.md#configuring-fiftyone).

You can also provide the optional `address` parameter to restrict the hostnames/IP addresses that can connect to your remote session. See [this page](installation__environments.md#restricting-app-address) for more information.
...
Run the [fiftyone app launch](https://docs.voxel51.com/cli/index.html#cli-fiftyone-app-launch) command in a terminal:
...
You can use the optional `--port` flag to choose the port of your remote machine on which to serve the App. The default is `5151`, which can also be customized via the `default_app_port` parameter of your [FiftyOne config](user_guide__config.md#configuring-fiftyone).
...
If you have FiftyOne installed on your local machine, you can [use the CLI](https://docs.voxel51.com/cli/index.html#cli-fiftyone-app-connect) to automatically configure port forwarding and open the App in your browser as follows:
...
Remote sessions are highly flexible. For example, you can connect to [multiple remote sessions](https://docs.voxel51.com/faq/index.html#faq-connect-to-multiple-remote-sessions) and run multiple remote sessions [from one machine](https://docs.voxel51.com/faq/index.html#faq-serve-multiple-remote-sessions).
...
By default, only label fields (excluding [heatmaps](https://docs.voxel51.com/user_guide/using_datasets.html#heatmaps) and [semantic segmentations](https://docs.voxel51.com/user_guide/using_datasets.html#semantic-segmentation)) are visible by default, but you can programmatically define a datasetâs a default configuration for these checkboxes by setting the [`active_fields`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") property of the [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config):
...
You can conveniently reset the active fields to their default state by setting [`active_fields`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") to `None`:
...
If you have [stored metadata](https://docs.voxel51.com/user_guide/using_datasets.html#storing-field-metadata) on your fields, then you can view this information in the App by hovering over field or attribute names in the Appâs sidebar:
...
Did you know? When you [declare custom attributes](https://docs.voxel51.com/user_guide/using_datasets.html#dynamic-attributes) on your datasetâs schema, they will automatically become filterable in the App!
...
The SDK provides a number of useful utilities for managing indexes on your datasets:
  ...
  * [`list_indexes()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.list_indexes "fiftyone.core.collections.SampleCollection.list_indexes") \- list all existing indexes

  * [`create_index()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.create_index "fiftyone.core.collections.SampleCollection.create_index") \- create a new index

  * [`drop_index()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.drop_index "fiftyone.core.collections.SampleCollection.drop_index") \- drop an existing index

  * [`get_index_information()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.get_index_information "fiftyone.core.collections.SampleCollection.get_index_information") \- get information about the existing indexes
...
Did you know? With [FiftyOne Enterprise](https://docs.voxel51.com/enterprise/index.html#fiftyone-enterprise) you can manage indexes natively in the App via the [Query Performance panel](enterprise__query_performance.md#query-performance).
...
Filtering by frame fields of video datasets is not directly optimizable by creating indexes. Instead, use [summary fields](https://docs.voxel51.com/user_guide/using_datasets.html#summary-fields) to efficiently query frame-level information on large video datasets.

Frame filtering in the Appâs grid view can be disabled by setting `disable_frame_filtering=True` in your [App config](user_guide__config.md#configuring-fiftyone-app).

For [grouped datasets](https://docs.voxel51.com/user_guide/groups.html#groups), you should create a compound index for each field you wish to filter by that includes the field itself and ends with the group slice name. This ensures grid results and counts are performant when filtering by that field and matching on the active `slice`:
...
![app-compound-indexes-for-query-performance](../_images/app-compound-indexes-for-query-performance.gif)
...
For [`ExcludeFields`](api__fiftyone.core.stages.md#fiftyone.core.stages.ExcludeFields "fiftyone.core.stages.ExcludeFields") and [`SelectFields`](api__fiftyone.core.stages.md#fiftyone.core.stages.SelectFields "fiftyone.core.stages.SelectFields"), index performance applies to all fields still present in the schema.

[`SelectGroupSlices`](api__fiftyone.core.stages.md#fiftyone.core.stages.SelectGroupSlices "fiftyone.core.stages.SelectGroupSlices") is query performant. Expect optimal performance when all slices are included in the flattened view.

The [`GroupBy`](api__fiftyone.core.stages.md#fiftyone.core.stages.GroupBy "fiftyone.core.stages.GroupBy") stage is a query performant stage when `order_by` and `order_by_key` values are provided and a compound index exists on the `group_by` and `order_by` fields with a unique constraint and at least one index exists that begins with the `order_by` field. Query performant fields then exist when they follow the `order_by` in a compound index.
...
![app-optimized-dynamic-groups](../_images/app-optimized-dynamic-groups.gif)
...
![app-unindexed-sidebar-results](../_images/app-unindexed-sidebar-results.gif)
...
You can also disable Query Performance by default for all datasets by setting `default_query_performance=False` in your [App config](user_guide__config.md#configuring-fiftyone-app).
...
You can also programmatically modify a datasetâs sidebar groups by editing the [`sidebar_groups`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") property of the [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config):
...
You can conveniently reset the sidebar groups to their default state by setting [`sidebar_groups`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") to `None`:
...
If a dataset has fields that do not appear in the datasetâs [`sidebar_groups`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") property, these fields will be dynamically assigned to default groups in the App at runtime.

## Using the view bar#

The view bar makes all of the powerful searching, sorting, and filtering operations [provided by dataset views](https://docs.voxel51.com/user_guide/using_views.html#using-views) available directly in the App.

Note

Any changes to the current view that you make in the view bar are automatically reflected in the [`DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") exposed by the [`Session.view`](api__fiftyone.core.session.md#fiftyone.core.session.Session.view "fiftyone.core.session.Session.view") property of the Appâs session object.

![app-views2](../_images/app-views2.gif)

## Grouping samples#

You can use the group action in the Appâs menu to [dynamically group](https://docs.voxel51.com/user_guide/using_views.html#view-groups) your samples by a field of your choice:
...
Alternatively, you can use the `Filter rule` tab to define a rule that is dynamically applied to the datasetâs [field metadata](https://docs.voxel51.com/user_guide/using_datasets.html#storing-field-metadata) each time the App loads to determine which fields to include in the sidebar.
...
To persist a color scheme, you can press `Save as default` to save the color scheme as the datasetâs default scheme, copy it via the modalâs JSON viewer, or access it programmatically via [`session.color_scheme`](api__fiftyone.core.session.md#fiftyone.core.session.Session.color_scheme "fiftyone.core.session.Session.color_scheme") as described below.
...
All | `Save as default` button | Save the current color scheme as the default for the current dataset. Note that this scheme can be viewed and/or modified [in Python](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config-color-scheme)  
...
`FIELD` | Use custom colors for specific field values | Allows you to specify custom colors to use to render annotations in this field based on the individual values that it takes. In the case of embedded document fields,you must also specify an attribute of each object. For example, color all [`Classification`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Classification "fiftyone.core.labels.Classification") instances whose `label` is `"car"` in `#FF0000`; [`Segmentation`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Segmentation "fiftyone.core.labels.Segmentation") instances whose `mask target integer` is `12` in `#FF0000`; [`Heatmap`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Heatmap "fiftyone.core.labels.Heatmap") instances using `hsv` colorscale.  
  
### Color schemes in Python#

You can also programmatically configure a sessionâs color scheme by creating [`ColorScheme`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.ColorScheme "fiftyone.core.odm.dataset.ColorScheme") instances in Python:
...
You can launch the App with a custom color scheme by passing the optional `color_scheme` parameter to [`launch_app()`](api__fiftyone.core.session.md#fiftyone.core.session.launch_app "fiftyone.core.session.launch_app"):
...
Once the App is launched, you can retrieve your current color scheme at any time via the [`session.color_scheme`](api__fiftyone.core.session.md#fiftyone.core.session.Session.color_scheme "fiftyone.core.session.Session.color_scheme") property:
...
Did you know? You can also configure default color schemes for [individual datasets](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config-color-scheme) via Python!
...
Did you know? You can also programmatically create, modify, and delete saved views [via Python](https://docs.voxel51.com/user_guide/using_views.html#saving-views)!

## Viewing a sample#

Click a sample to open an expanded view of the sample. This modal also contains information about the fields of the [`Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample") and allows you to access the raw JSON description of the sample.

![app-expanded](../_images/app-expanded.gif)

If your labels contain many [dynamic attributes](https://docs.voxel51.com/user_guide/using_datasets.html#dynamic-attributes), you may find it helpful to configure which attributes are shown in the tooltip. To do so, press `ctrl` while hovering over a label to lock the tooltip in-place and then use the show/hide buttons to customize the display.
...
When working in [Jupyter/Colab notebooks](installation__environments.md#notebooks), you can hold down the `SHIFT` key when zoom-scrolling or using the arrow keys to navigate between samples/labels to restrict your inputs to the App and thus prevent them from also affecting your browser window.
...
The 3D visualizer allows you to interactively visualize [3D samples](https://docs.voxel51.com/user_guide/using_datasets.html#d-datasets) or [point cloud samples](https://docs.voxel51.com/user_guide/using_datasets.html#point-cloud-datasets) along with any associated [3D detections](https://docs.voxel51.com/user_guide/using_datasets.html#d-detections) and [3D polylines](https://docs.voxel51.com/user_guide/using_datasets.html#d-polylines):
...
![pcd-dynamic-coloring](../_images/pcd-dynamic-coloring.gif)
...
Colormap selection follows this precedence order:
  ...
  2. Colormap defined in the `colorscales` property of the [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config) for the specific attribute
...
You can define default colormaps for point cloud attributes of a dataset by configuring them in the [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config). You must use the prefix `::fo3d::pcd::` followed by the attribute name in the `path` field. For example, to define a colormap for the `lidar_id` attribute, the path should be `::fo3d::pcd::lidar_id`:
...
![thresholding](../_images/pcd-thresholding.gif)
...
When you load 3D collections in the App, any [3D detections](https://docs.voxel51.com/user_guide/using_datasets.html#d-detections) and [3D polylines](https://docs.voxel51.com/user_guide/using_datasets.html#d-polylines) fields will be visualized in the grid using an orthographic projection (onto the xy plane by default).

In addition, if you have populated [orthographic projection images](https://docs.voxel51.com/user_guide/using_datasets.html#orthographic-projection-images) on your dataset, the projection images will be rendered for each sample in the grid:
...
The 3D visualizer can be configured by including any subset of the settings shown below under the `plugins.3d` key of your [App config](user_guide__config.md#configuring-fiftyone-app):
...
You can also store dataset-specific plugin settings by storing any subset of the above values on a [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config):
...
Dataset-specific plugin settings will override any settings from your [global App config](user_guide__config.md#configuring-fiftyone-app).
...
In FiftyOne Enterprise, only users with [Can manage](enterprise__roles_and_permissions.md#enterprise-can-manage) dataset access can access the Schema Manager.

For more information on FiftyOneâs in-App annotation features, [visit this User Guide!](user_guide__annotation.md#in-app-annotation)
...
In [video datasets](https://docs.voxel51.com/user_guide/using_datasets.html#linking-labels-across-frames), label correspondence helps track objects over time.
...
![labels-correspondence-video](../_images/labels-correspondence-video.gif)

### Grouped datasets#

In [grouped datasets](https://docs.voxel51.com/user_guide/groups.html#linking-labels-across-slices), label correspondence links the same object viewed from different perspectives (group slices).
...
![labels-correspondence-groups](../_images/labels-correspondence-groups.gif)
...
FiftyOne natively includes the following Panels:
  ...
  * Embeddings panel: a canvas for working with [embeddings visualizations](brain.md#brain-embeddings-visualization)

  * Model Evaluation panel: interactively analyze and visualize your modelâs performance

  * Map panel: visualizes the geolocation data of datasets that have a [`GeoLocation`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.GeoLocation "fiftyone.core.labels.GeoLocation") field
...
You can also configure custom Panels [via plugins](plugins__overview.md#fiftyone-plugins)!
...
You can launch the App with an initial spaces layout by passing the optional `spaces` parameter to [`launch_app()`](api__fiftyone.core.session.md#fiftyone.core.session.launch_app "fiftyone.core.session.launch_app"):
...
Once the App is launched, you can retrieve your current layout at any time via the [`session.spaces`](api__fiftyone.core.session.md#fiftyone.core.session.Session.spaces "fiftyone.core.session.Session.spaces") property:
    
    
    1print(session.spaces)
    

You can also programmatically configure the Appâs current layout by setting [`session.spaces`](api__fiftyone.core.session.md#fiftyone.core.session.Session.spaces "fiftyone.core.session.Session.spaces") to any valid `Space` instance:
...
Inspecting [`session.spaces`](api__fiftyone.core.session.md#fiftyone.core.session.Session.spaces "fiftyone.core.session.Session.spaces") of a session whose Spaces layout youâve configured in the App is a convenient way to discover the available state options for each Panel type!

You can reset your spaces to their default state by setting [`session.spaces`](api__fiftyone.core.session.md#fiftyone.core.session.Session.spaces "fiftyone.core.session.Session.spaces") to None:
...
Use [`save_workspace()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.save_workspace "fiftyone.core.dataset.Dataset.save_workspace") to create a new saved workspace with a name of your choice:
...
Pro tip! You can save your current spaces layout in the App via [`session.spaces`](api__fiftyone.core.session.md#fiftyone.core.session.Session.spaces "fiftyone.core.session.Session.spaces"):
...
Then in a future session you can load the workspace by name with [`load_workspace()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.load_workspace "fiftyone.core.dataset.Dataset.load_workspace"):
...
Saved workspaces have certain editable metadata such as a name, description, and color that you can view via [`get_workspace_info()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.get_workspace_info "fiftyone.core.dataset.Dataset.get_workspace_info") and update via [`update_workspace_info()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.get_workspace_info "fiftyone.core.dataset.Dataset.get_workspace_info"):
...
You can also use [`list_workspaces()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.list_workspaces "fiftyone.core.dataset.Dataset.list_workspaces"), [`has_workspace()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.has_workspace "fiftyone.core.dataset.Dataset.has_workspace"), and [`delete_workspace()`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.delete_workspace "fiftyone.core.dataset.Dataset.delete_workspace") to manage your saved workspaces.
...
When you load a dataset in the App that contains an [embeddings visualization](brain.md#brain-embeddings-visualization), you can open the Embeddings panel to visualize and interactively explore a scatterplot of the embeddings in the App:
...
Use the two menus in the upper-left corner of the Panel to configure your plot:
  ...
  * **Brain key** : the brain key associated with the [`compute_visualization()`](https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_visualization "fiftyone.brain.compute_visualization") run to display
...
Did you know? With [FiftyOne Enterprise](https://docs.voxel51.com/enterprise/index.html#fiftyone-enterprise) you can generate embeddings visualizations natively from the App [in the background](plugins__using_plugins.md#delegated-operations) while you work.
...
The Embeddings panel supports the following `state` parameters:
  ...
  * **brainResult** : the brain key associated with the [`compute_visualization()`](https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_visualization "fiftyone.brain.compute_visualization") run to display
...
When you load a dataset in the App that has one or more [similarity indexes](brain.md#brain-similarity), you can open the Similarity Search panel to create, manage, and revisit similarity searches on the dataset.

To get started, compute a similarity index on your dataset using [`compute_similarity()`](https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_similarity "fiftyone.brain.compute_similarity"):
...
Refer to the [Brain guide](brain.md#brain-similarity) for more information on supported backends (sklearn, Qdrant, Pinecone, MongoDB, etc.), distance metrics, and using custom or precomputed embeddings.
...
![similarity-panel-home-page](../_images/app-similarity-panel-home.png)
...
![similarity-panel-manage-runs](../_images/app-similarity-panel-manage-runs.gif)
...
![similarity-panel-index-page](../_images/app-similarity-panel-index-page.gif)
...
![similarity-panel-new-search-positive-negative](../_images/app-similarity-panel-new-search.png)
...
If the selected similarity index was built on [object patches](brain.md#brain-object-similarity) (e.g., Detections or Polylines), the search will return patch results; otherwise it returns sample results.

### Delegated execution#

Triggered from the popover, similarity searches run immediately on the App server by default. If your deployment supports [delegated operations](plugins__using_plugins.md#delegated-operations), you can choose to run the search on a worker pod instead by selecting **Delegate** as the execution mode. This is useful for a large number of results or large datasets.
...
![similarity-popover-open-panel](../_images/app-similarity-popover.gif)
...
Whenever one or more images are selected in the App, the similarity icon appears above the grid. If you have indexed the dataset by [image similarity](brain.md#brain-image-similarity), you can click the icon to sort by similarity to your current selection.
...
Whenever one or more labels or patches are selected in the App, the similarity icon appears above the sample grid. If you have indexed the dataset by [object similarity](brain.md#brain-object-similarity), you can sort by similarity to your current selection.
...
If you have indexed your dataset with a model that [supports text queries](brain.md#brain-similarity-text), you can use the similarity popover to search for images (or object patches) of interest via arbitrary text queries. Simply type your query into the text input field and press search.

![text-similarity](../_images/brain-text-similarity.gif)

## Model Evaluation panel **NEW**#

When you load a dataset in the App that contains one or more [evaluations](user_guide__evaluation.md#evaluating-models), you can open the Model Evaluation panel to visualize and interactively explore the evaluation results in the App:
...
Did you know? With [FiftyOne Enterprise](https://docs.voxel51.com/enterprise/index.html#fiftyone-enterprise) you can execute model evaluations natively from the App [in the background](plugins__using_plugins.md#delegated-operations) while you work.
...
Scenario analysis is available for all [evaluations](user_guide__evaluation.md#evaluating-models) by clicking on the Scenario Analysis tab of the Model Evaluation panel.
...
![scenario-analysis-create-cta](../_images/scenario-analysis-create-cta.png)
...
![scenario-analysis-create-empty](../_images/scenario-analysis-create-empty.png)
...
![scenario-analysis-create-sample-field](../_images/scenario-analysis-create-sample-field.gif)
...
![scenario-analysis-create-label-attribute](../_images/scenario-analysis-create-label-attribute.gif)
...
Selecting saved views allows you to define a scenario where each subset contains the ground truth labels in a specified [saved view](https://docs.voxel51.com/user_guide/using_views.html#saving-views).
...
![scenario-analysis-create-saved-views](../_images/scenario-analysis-create-saved-views.gif)
...
![scenario-analysis-create-custom-code](../_images/scenario-analysis-create-custom-code.gif)
...
Refer to the [`ViewExpression`](api__fiftyone.core.expressions.md#fiftyone.core.expressions.ViewExpression "fiftyone.core.expressions.ViewExpression") docs for a full list of supported operations when using expressions to define subsets.
...
![scenario-analysis-graphs](../_images/scenario-analysis-graphs.gif)
...
![scenario-analysis-callbacks](../_images/scenario-analysis-callbacks.gif)
...
![scenario-analysis-tables](../_images/scenario-analysis-tables.gif)
...
Additionally, the map UI can be configured by including any subset of the settings shown below under the `plugins.map` key of your [App config](user_guide__config.md#configuring-fiftyone-app):
...
The Histograms panel in the App lets you visualize different statistics about the fields of your dataset.
  ...
  * The `Labels` mode shows the class distributions for each [labels field](https://docs.voxel51.com/user_guide/using_datasets.html#using-labels) that youâve added to your dataset. For example, you may have histograms of ground truth labels and one more sets of model predictions

  * The `Other fields` mode shows distributions for numeric (integer or float) or categorical (e.g., string) [primitive fields](https://docs.voxel51.com/user_guide/using_datasets.html#adding-sample-fields) that youâve added to your dataset. For example, if you computed [uniqueness](brain.md#brain-image-uniqueness) on your dataset, a histogram of uniqueness values will be available under this mode.
...
The statistics in the plots automatically update to reflect the current [view](https://docs.voxel51.com/user_guide/using_views.html#using-views) that you have loaded in the App!
...
As previously explained, the [`Session`](api__fiftyone.core.session.md#fiftyone.core.session.Session "fiftyone.core.session.Session") object created when you launch the App lets you interact with the App from your Python process.
...
You can also access the [`Session.selected`](api__fiftyone.core.session.md#fiftyone.core.session.Session.selected "fiftyone.core.session.Session.selected") property of your session to retrieve the IDs of the currently selected samples in the App:
...
You can also select individual labels within samples in the App, which is useful for a variety of reasons:
  ...
  * Access the data for selected labels from Python, for example by creating a [`DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") that includes/excludes the selected labels
...
You can also access the [`Session.selected_labels`](api__fiftyone.core.session.md#fiftyone.core.session.Session.selected_labels "fiftyone.core.session.Session.selected_labels") property of your session to retrieve information about the currently selected labels in the App:
...
![app-linked-label-selections](../_images/app-linked-label-selections.gif)
...
![app-persistent-selections](../_images/app-persistent-selections.gif)
...
![app-persistent-selections-clear-app-menu](../_images/app-persistent-selections-clear-app-menu.png)
...
![app-persistent-selections-clear-sample-menu](../_images/app-persistent-selections-clear-sample-menu.png)

## Tags and tagging#

Tagging is a first-class citizen in FiftyOne, as both [`Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample") and [`Label`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Label "fiftyone.core.labels.Label") instances have a `tags` attribute that you can use to store arbitrary string tags for your data.

The FiftyOne API provides methods like [`tag_samples()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.tag_samples "fiftyone.core.collections.SampleCollection.tag_samples") and [`tag_labels()`](api__fiftyone.core.collections.md#fiftyone.core.collections.SampleCollection.tag_labels "fiftyone.core.collections.SampleCollection.tag_labels") that you can use to programmatically manage the tags on your dataset. However, the App also provides a convenient UI for interactively adding, removing, and filtering by [`Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample") and [`Label`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Label "fiftyone.core.labels.Label") tags.
...
For example, take the following steps to tag all labels in the `predictions` field of a dataset:
  ...
  * Make sure that `predictions` is the only [`Label`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Label "fiftyone.core.labels.Label") field checked in the filters sidebar
...
Whenever you load a dataset in the App that contains label list fields in [`Detections`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Detections "fiftyone.core.labels.Detections") or [`Polylines`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Polylines "fiftyone.core.labels.Polylines") format, you can use the patches menu to create a view into your data that contains one sample per object patch in a specified label field of your dataset.

To switch to patches view, simply click the patches icon above the sample grid in the App, toggle to the `Labels` submenu, and then choose the field whose object patches you want to view. After you make a selection, a new [`ToPatches`](api__fiftyone.core.stages.md#fiftyone.core.stages.ToPatches "fiftyone.core.stages.ToPatches") view stage will be appended to the view bar and your view will be updated to show the patches.
...
One notable exception is that tagging or untagging patches themselves (as opposed to their labels) will not affect the sample tags of the underlying [`Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample").

Note

Did you know? You can construct object patches views programmatically via [dataset views](https://docs.voxel51.com/user_guide/using_views.html#object-patches-views)!

## Viewing evaluation patches#

Whenever you load a dataset in the App that contains object detections on which you have [run evaluation](user_guide__evaluation.md#evaluating-detections), you can use the patches menu to create a view into your data that contains one sample for each true positive, false positive, and false negative example.

To switch to evaluation patches view, simply click the patches icon above the sample grid in the App, toggle to the `Evaluations` submenu, and then choose the `eval_key` under which you saved the evaluation results that you want view. After you make a selection, a new [`ToEvaluationPatches`](api__fiftyone.core.stages.md#fiftyone.core.stages.ToEvaluationPatches "fiftyone.core.stages.ToEvaluationPatches") view stage will be appended to the view bar and your view will be updated to show the evaluation patches!
...
Whenever you load a video dataset in the App that contains [`TemporalDetection`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.TemporalDetection "fiftyone.core.labels.TemporalDetection") labels or frame-level label lists such as [`Detections`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Detections "fiftyone.core.labels.Detections"), you can use the patches menu to create a view into your data that contains one sample per clip defined by a specified label field of your dataset.

To switch to clips view, simply click the patches icon above the sample grid in the App, toggle to the `Labels` submenu, and then choose the field whose clips you want to view. After you make a selection, a new [`ToClips`](api__fiftyone.core.stages.md#fiftyone.core.stages.ToClips "fiftyone.core.stages.ToClips") view stage will be appended to the view bar and your view will be updated to show the clips.
...
Creating a clips view for a frame-level label list field such as [`Detections`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Detections "fiftyone.core.labels.Detections") will contain one sample per contiguous range of frames that contains at least one label in the specified field:
...
See [this section](https://docs.voxel51.com/user_guide/using_views.html#clip-views) for more information about defining clip views.
...
One notable exception is that tagging or untagging clips themselves (as opposed to their labels) will not affect the sample tags of the underlying [`Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample").

Note

Did you know? You can construct clip views programmatically via [dataset views](https://docs.voxel51.com/user_guide/using_views.html#clip-views)!
...
We can expose the thumbnail images to the App by modifying the [datasetâs App config](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config):
...
Adding `thumbnail_path` to the [`media_fields`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") property adds it to the `Media Field` selector under the Appâs settings menu, and setting the [`grid_media_field`](api__fiftyone.core.odm.dataset.md#fiftyone.core.odm.dataset.DatasetAppConfig "fiftyone.core.odm.dataset.DatasetAppConfig") property to `thumbnail_path` instructs the App to use the thumbnail images by default in the grid view:
...
The order of precedence is:
  ...
  1. Any changes that you make to the [`session.config`](api__fiftyone.core.session.md#fiftyone.core.session.Session.config "fiftyone.core.session.Session.config") of a live session

  2. Any settings stored in a datasetâs [`app_config`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.app_config "fiftyone.core.dataset.Dataset.app_config")

  3. Settings from your [global App config](user_guide__config.md#configuring-fiftyone-app)
...
FiftyOne provides a [global App config](user_guide__config.md#configuring-fiftyone-app) that you can use to customize the default App behavior for all sessions and datasets on your machine.
...
You can configure a live [`Session`](api__fiftyone.core.session.md#fiftyone.core.session.Session "fiftyone.core.session.Session") by editing its [`session.config`](api__fiftyone.core.session.md#fiftyone.core.session.Session.config "fiftyone.core.session.Session.config") property and calling [`session.refresh()`](api__fiftyone.core.session.md#fiftyone.core.session.Session.refresh "fiftyone.core.session.Session.refresh") to apply the changes:
...
Datasets also provide an [app_config property](https://docs.voxel51.com/user_guide/using_datasets.html#dataset-app-config) that you can use to customize the behavior of the App for that particular dataset:
...
Any settings stored in a datasetâs [`app_config`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset.app_config "fiftyone.core.dataset.Dataset.app_config") will override the corresponding settings from your [global App config](user_guide__config.md#configuring-fiftyone-app).
