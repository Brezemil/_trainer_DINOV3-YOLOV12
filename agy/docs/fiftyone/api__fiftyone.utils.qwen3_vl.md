---
source_url: https://docs.voxel51.com/api/fiftyone.utils.qwen3_vl.html
---

# fiftyone.utils.qwen3_vl#

[Qwen3-VL](https://huggingface.co/collections/Qwen/qwen3-vl) wrapper for the FiftyOne Model Zoo.
...
`Qwen3VLOutputProcessor`([classes]) | Output processor for Qwen3-VL detection models.  
---|---  
`Qwen3VLModelConfig`(d) | Configuration for running a `Qwen3VLModel`.  
`Qwen3VLModel`(config) | Wrapper for running inference with Qwen3-VL models.  
  
class fiftyone.utils.qwen3_vl.Qwen3VLOutputProcessor(_classes =None_, _** kwargs_)#
...
Parses JSON bounding box output and converts to [`fiftyone.core.labels.Detections`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Detections "fiftyone.core.labels.Detections") instances.

class fiftyone.utils.qwen3_vl.Qwen3VLModelConfig(_d_)#
...
Configuration for running a `Qwen3VLModel`.

Parameters:
    

  * **name_or_path** (_"Qwen/Qwen3-VL-2B-Instruct"_) â the HuggingFace model path

  * **prompt** (_None_) â the detection prompt; if None, uses default

  * **classes** (_None_) â list of classes to detect; if provided, added to prompt

  * **max_new_tokens** (_4096_) â maximum tokens to generate

  * **embedding_dim** (_None_) â output embedding dimension for MRL truncation; if None, uses full model dimension (2048 for 2B, 3584 for 8B)

  * **normalize_embeddings** (_True_) â whether to L2 normalize embeddings

  * **video_fps** (_2.0_) â frame sampling rate for video inputs; Qwen3-VLâs default is 2.0 FPS. Lower values = fewer frames = faster

  * **max_video_frames** (_128_) â maximum frames to sample from a video; prevents OOM on long videos. Matches qwen-vl-utils MAX_FRAMES.

  * **mode** (_None_) â the media type mode, âimageâ or âvideoâ; if None, defaults to the datasetâs media type at inference time
...
class fiftyone.utils.qwen3_vl.Qwen3VLModel(_config_)#
    

Bases: [`TorchImageModel`](api__fiftyone.utils.torch.md#fiftyone.utils.torch.TorchImageModel "fiftyone.utils.torch.TorchImageModel"), [`EmbeddingsMixin`](api__fiftyone.core.models.md#fiftyone.core.models.EmbeddingsMixin "fiftyone.core.models.EmbeddingsMixin"), [`PromptMixin`](api__fiftyone.core.models.md#fiftyone.core.models.PromptMixin "fiftyone.core.models.PromptMixin")
...
**arg** â a PIL image, numpy array, torch tensor, or an active (entered) `eta.core.video.FFmpegVideoReader` context manager
...
**args** â an iterable of PIL images, numpy arrays, or torch tensors
