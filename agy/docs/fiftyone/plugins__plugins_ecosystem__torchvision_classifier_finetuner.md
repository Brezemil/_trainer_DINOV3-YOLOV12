---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/torchvision_classifier_finetuner.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/sherpan/torchvision-classifier-finetuner)

# torchvision-classifier-finetuner#

A [FiftyOne plugin](https://docs.voxel51.com/plugins/index.html) with two operators: one to fine-tune a pretrained torchvision image classification model on any labeled FiftyOne dataset, and one to run inference with the saved checkpoint â all directly from the UI or Python API, no training boilerplate required.

![Fine-tuner demo](https://raw.githubusercontent.com/sherpan/torchvision-classifier-finetuner/main/screenshots/ft_gif.gif)
...
The plugin performs transfer learning on top of a pretrained image classification backbone. Given a FiftyOne dataset with a `Classification` label field, it will:
  ...
  2. Auto-create an 80/20 train/val split (by tagging samples) if one doesnât already exist.
...
The companion inference operator loads the checkpoint and writes `fo.Classification` predictions directly onto your dataset. Because predictions are stored as native FiftyOne labels, they immediately unlock [FiftyOneâs model evaluation suite](https://docs.voxel51.com/user_guide/evaluation.html) â letting you compute per-class metrics, visualize confusion matrices, and sort/filter by confidence or correctness from the app.
...
Once you have a fine-tuned checkpoint, use the **Run Torchvision Classifier Inference** operator to write predictions back to any FiftyOne view â without leaving the app.
...
Parameter | Type | Default | Description  
---|---|---|---  
`model_uri` | file | â | Path to the `.pt` checkpoint (local, `gs://`, or `s3://`)  
...
The checkpoint is self-contained â it stores the architecture name, class labels, and image size, so you never need to re-specify them at inference time.

* * *

## Customizing for your use case#

The plugin is split into focused modules â each covering one concern. Edit only the file relevant to what you want to change:

Goal | File to edit  
---|---  
Add a new backbone (ViT, ConvNeXt, etc.) | `models.py`  
Change data augmentation | `transforms.py`  
Swap loss function (label smoothing, focal loss) | `__init__.py` â one line in `execute()`  
Swap optimizer or LR scheduler | `__init__.py` â one line in `execute()`  
Change train/val split ratio or strategy | `__init__.py` â `execute()` split section  
Change how samples are filtered or loaded | `dataset.py`  
  
### File responsibilities#

  * **`models.py`** â `build_model()` \+ `SUPPORTED_MODELS` dict. The UI dropdown auto-populates from `SUPPORTED_MODELS`, so adding a key here is all it takes to expose a new architecture.

  * **`transforms.py`** â `get_transforms()`. Augmentation changes stay fully isolated from training logic.

  * **`trainer.py`** â `train()` function. Accepts model, loaders, criterion, optimizer, scheduler, epochs, device, and ctx. Returns `best_val_acc` and `best_state`.

  * **`dataset.py`** â `FiftyOneClassificationDataset`. Handles label filtering and mapping between FiftyOne sample IDs and integer class indices.

  * **`__init__.py`** â Thin operator shell. `execute()` wires together the modules: discovers classes, handles the train/val split, builds dataloaders, constructs criterion/optimizer/scheduler, calls `trainer.train()`, and saves the checkpoint.
