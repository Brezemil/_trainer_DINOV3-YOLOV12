---
source_url: https://docs.voxel51.com/api/fiftyone.brain.similarity.html
---
...
`compute_similarity`(samples,Â patches_field,Â ...) | See `fiftyone/brain/__init__.py`.  
---|---  
  
**Classes:**

`SimilarityConfig`([embeddings_field,Â model,Â ...]) | Similarity configuration.  
---|---  
`Similarity`(config) | Base class for similarity factories.  
`SimilarityIndex`(samples,Â config,Â brain_key) | Base class for similarity indexes.  
`DuplicatesMixin`() | Mixin for `SimilarityIndex` instances that support duplicate detection operations.  
  
fiftyone.brain.similarity.compute_similarity(_samples_ , _patches_field_ , _roi_field_ , _embeddings_ , _brain_key_ , _model_ , _model_kwargs_ , _force_square_ , _alpha_ , _batch_size_ , _num_workers_ , _skip_failures_ , _progress_ , _backend_ , _** kwargs_)#
    

See `fiftyone/brain/__init__.py`.

class fiftyone.brain.similarity.SimilarityConfig(_embeddings_field =None_, _model =None_, _model_kwargs =None_, _patches_field =None_, _roi_field =None_, _supports_prompts =None_, _** kwargs_)#
...
Parameters:
  ...
  * **embeddings_field** (_None_) â the sample field containing the embeddings, if one was provided

  * **model** (_None_) â the [`fiftyone.core.models.Model`](api__fiftyone.core.models.md#fiftyone.core.models.Model "fiftyone.core.models.Model") or name of the zoo model that was used to compute embeddings, if known

  * **model_kwargs** (_None_) â a dictionary of optional keyword arguments to pass to the modelâs `Config` when a model name is provided

  * **patches_field** (_None_) â the sample field defining the patches being analyzed, if any

  * **roi_field** (_None_) â the sample field defining a region of interest within each image to use to compute embeddings, if any

  * **supports_prompts** (_False_) â whether this run supports prompt queries
...
class fiftyone.brain.similarity.Similarity(_config_)#
...
class fiftyone.brain.similarity.SimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `SimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `Similarity` backend
