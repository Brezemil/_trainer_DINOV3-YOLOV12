---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/audio_retrieval.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/jacobmarks/audio-retrieval-plugin)

# Audio-to-Image Search Plugin#
...
## `create_imagebind_index`#
  ...
  * Creates an index for the dataset using the ImageBind embedding model. This operation can take a little while to run, so it is recommended to run it in delegated execution mode. To do so, check the `Delegated` box in the operatorâs modal, and then in a terminal run:
...
Before you can use the plugin, you will need to create an account on [Replicate.com](https://replicate.com/). Once you have created an account, you can create an API token, and then add this token as an environment variable:
