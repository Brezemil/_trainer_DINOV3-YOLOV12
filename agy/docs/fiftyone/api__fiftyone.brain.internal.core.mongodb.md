---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.mongodb.html
---
...
`MongoDBSimilarityConfig`([index_name,Â metric]) | Configuration for a MongoDB similarity instance.  
---|---  
`MongoDBSimilarity`(config) | MongoDB similarity factory.  
`MongoDBSimilarityIndex`(samples,Â config,Â ...) | Class for interacting with MongoDB similarity indexes.  
  
class fiftyone.brain.internal.core.mongodb.MongoDBSimilarityConfig(_index_name =None_, _metric ='cosine'_, _** kwargs_)#
...
Parameters:
  ...
  * **index_name** (_None_) â the name of the MongoDB vector index to use or create. If none is provided, a new index will be created
...
class fiftyone.brain.internal.core.mongodb.MongoDBSimilarity(_config_)#
...
**config** â a `MongoDBSimilarityConfig`
...
class fiftyone.brain.internal.core.mongodb.MongoDBSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `MongoDBSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `MongoDBSimilarity` instance
...
`ready` | Returns True/False whether the vector search index is ready to be queried.  
...
Returns True/False whether the vector search index is ready to be queried.
