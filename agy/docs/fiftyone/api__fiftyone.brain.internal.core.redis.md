---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.redis.html
---
...
`RedisSimilarityConfig`([index_name,Â metric,Â ...]) | Configuration for the Redis similarity backend.  
---|---  
`RedisSimilarity`(config) | Redis similarity factory.  
`RedisSimilarityIndex`(samples,Â config,Â brain_key) | Class for interacting with Redis similarity indexes.  
  
class fiftyone.brain.internal.core.redis.RedisSimilarityConfig(_index_name =None_, _metric ='cosine'_, _algorithm ='FLAT'_, _host ='localhost'_, _port =6379_, _db =0_, _username =None_, _password =None_, _** kwargs_)#
...
Parameters:
  ...
  * **index_name** (_None_) â the name of a Redis index to use or create. If none is provided, a new index will be created

  * **metric** (_"cosine"_) â the embedding distance metric to use when creating a new index. Supported values are `("cosine", "dotproduct", "euclidean")`

  * **algorithm** (_"FLAT"_) â the search algorithm to use. The supported values are `("FLAT", "HNSW")`

  * **host** (_"localhost"_) â the host to use

  * **port** (_6379_) â the port to use

  * **db** (_0_) â the database to use
...
`load_credentials`([host,Â port,Â db,Â username,Â ...]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
load_credentials(_host =None_, _port =None_, _db =None_, _username =None_, _password =None_)#
...
class fiftyone.brain.internal.core.redis.RedisSimilarity(_config_)#
...
**config** â a `RedisSimilarityConfig`
...
class fiftyone.brain.internal.core.redis.RedisSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `RedisSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `RedisSimilarity` instance
...
`client` | The `redis.client.Redis` instance for this index.  
---|---  
