---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/molmo_point.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/molmo_point)

# MolmoPoint â FiftyOne Remote Zoo Model#

![image/png](https://raw.githubusercontent.com/harpreetsahota204/molmo_point/main/molmo_point_image.gif)

MolmoPoint is a vision-language model from the Allen Institute for AI that locates and tracks objects in images and videos by **pointing** â returning precise pixel coordinates â rather than generating bounding boxes. Given a natural language description like `"Point to the boats"`, MolmoPoint finds every matching instance and returns a set of keypoints, one per object.

## What makes it different#

Most grounding models predict coordinates as text output (e.g. `"[412, 308]"`), which forces the model to memorise an arbitrary coordinate system and uses many tokens per point. MolmoPoint instead emits special **grounding tokens** that directly attend to the imageâs visual tokens and select the patch that contains the target, then refines the prediction sub-patch by sub-patch down to ~5 pixel precision. This gives it:

  * **Higher accuracy** â state-of-the-art on PointBench (70.7%) and PixMo-Points, beating much larger models including Gemini 2.5 Pro

  * **Faster inference** â 3 tokens per point instead of ~8, meaning faster decoding especially when many objects are present

  * **Consistent resolution** â ~5 pixel precision regardless of input image size, including high-resolution images
...
MolmoPoint is a strong choice when you need to **locate objects without labelled bounding boxes**. Good use cases include:
  ...
  * **Zero-shot object localization** â find any object described in natural language, with no prior annotation or fine-tuning

  * **Counting via pointing** â the model returns one point per instance, so the count is simply the number of returned keypoints

  * **Referring expressions** â point to objects described by relationship or attribute, e.g. `"the red car on the left"`, `"the person holding an umbrella"`

  * **Weak supervision bootstrapping** â use the returned keypoints as rough center-point annotations to seed a downstream detector or segmentation model

  * **GUI interaction & automation** â `MolmoPoint-Img-8B` finds buttons, fields, and other interactive elements in screenshots by their natural-language description




MolmoPoint is **not** a detection model â it returns center points, not bounding boxes. If you need tight boxes, consider using the keypoints as seeds for a downstream model.
...
`apply_model` stores a [`fo.Keypoints`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Keypoints) label on each sample at the field name you specify (e.g. `"molmo_points"`). Each `Keypoints` object contains one [`fo.Keypoint`](https://docs.voxel51.com/api/fiftyone.core.labels.html#fiftyone.core.labels.Keypoint) per located instance, with:
  ...
  * **`label`** â the object description that produced this point (e.g. `"person"`)

  * **`points`** â a single `[[x, y]]` coordinate pair, normalized to `[0, 1]` relative to the image dimensions
...
The model converts the timestamps it returns into FiftyOne frame numbers using the videoâs frame rate. Without metadata, it falls back to 30 fps with a warning:
...
Each `fo.Keypoint` contains:
  ...
  * **`label`** â the object name from your prompt (e.g. `"person"`)

  * **`index`** â integer object ID from the model â the same object keeps the same ID across frames, making it useful for tracking identity over time

  * **`points`** â a single `[[x, y]]` coordinate pair, normalized to `[0, 1]`
...
The model stays on the GPU. All inference parameters can be changed freely between runs:
    ...
    # Switch to pointing â max_fps automatically updates to 2
    model.operation = "pointing"
    
    # Explicitly pin max_fps â won't change when you switch operation
    model.max_fps = 5
...
The relationship between `max_fps` and interpolation: higher `max_fps` â denser keyframes â less interpolation needed. Lower `max_fps` â sparser keyframes â more frames filled by interpolation. The `interp_max_gap` threshold is a safety net that prevents interpolation from silently bridging gaps where the object was genuinely absent.

### Per-sample prompts (video)#

Works the same as images â store a list of object names on each sample and pass the field name via `prompt_field`:
...
`MolmoPoint-Vid-4B` is a smaller model optimised specifically for video. Swap it in by changing the model name â everything else is identical:
