---
source_url: https://docs.voxel51.com/api/fiftyone.core.patches.html
---
...
`PatchView`(doc,Â view[,Â selected_fields,Â ...]) | A patch in a `PatchesView`.  
---|---  
`EvaluationPatchView`(doc,Â view[,Â ...]) | A patch in an `EvaluationPatchesView`.  
`PatchesView`(source_collection,Â ...[,Â ...]) | A [`fiftyone.core.view.DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") of patches from a [`fiftyone.core.dataset.Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset").  
`EvaluationPatchesView`(source_collection,Â ...) | A [`fiftyone.core.view.DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") containing evaluation patches from a [`fiftyone.core.dataset.Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset").  
  
**Functions:**

`make_patches_dataset`(sample_collection,Â field) | Creates a dataset that contains one sample per object patch in the specified field of the collection.  
---|---  
`make_evaluation_patches_dataset`(...[,Â ...]) | Creates a dataset based on the results of the evaluation with the given key that contains one sample for each true positive, false positive, and false negative example in the input collection, respectively.  
  
class fiftyone.core.patches.PatchView(_doc_ , _view_ , _selected_fields =None_, _excluded_fields =None_, _filtered_fields =None_)#
...
`PatchView` instances should not be created manually; they are generated by iterating over `PatchesView` instances.
...
Parameters:
  ...
  * **view** â the `PatchesView` that the patch belongs to
...
class fiftyone.core.patches.EvaluationPatchView(_doc_ , _view_ , _selected_fields =None_, _excluded_fields =None_, _filtered_fields =None_)#
...
Parameters:
  ...
  * **view** â the `EvaluationPatchesView` that the patch belongs to
...
class fiftyone.core.patches.PatchesView(_source_collection_ , _patches_stage_ , _patches_dataset_ , __stages =None_, __media_type =None_, __name =None_)#
    

Bases: `_PatchesView`

A [`fiftyone.core.view.DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") of patches from a [`fiftyone.core.dataset.Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset").

Patches views contain an ordered collection of patch samples, each of which contains a subset of a sample of the parent dataset corresponding to a single object or logical grouping of objects.

Patches retrieved from patches views are returned as `PatchView` objects.
...
Parameters:
  ...
  * **patches_stage** â the [`fiftyone.core.stages.ToPatches`](api__fiftyone.core.stages.md#fiftyone.core.stages.ToPatches "fiftyone.core.stages.ToPatches") stage that defines how the patches were extracted

  * **patches_dataset** â the [`fiftyone.core.dataset.Dataset`](https://docs.voxel51.com/api/fiftyone.core.dataset.html#fiftyone.core.dataset.Dataset "fiftyone.core.dataset.Dataset") that serves the patches in this view
...
`patches_field` | The field from which the patches in this view were extracted.  
---|---  
...
`keep`() | Deletes all patches that are **not** in this view from the underlying dataset.  
`keep_fields`() | Deletes all patch field(s) that have been excluded from this view from the underlying dataset.  
...
`save`([fields]) | Saves the patches in this view to the underlying dataset.  
...
class fiftyone.core.patches.EvaluationPatchesView(_source_collection_ , _patches_stage_ , _patches_dataset_ , __stages =None_, __media_type =None_, __name =None_)#
...
Evaluation patches views contain an ordered collection of evaluation examples, each of which contains the ground truth and/or predicted labels for a true positive, false positive, or false negative example from an evaluation run on the underlying dataset.

Patches retrieved from patches views are returned as `EvaluationPatchView` objects.
...
Parameters:
  ...
  * **patches_stage** â the [`fiftyone.core.stages.ToEvaluationPatches`](api__fiftyone.core.stages.md#fiftyone.core.stages.ToEvaluationPatches "fiftyone.core.stages.ToEvaluationPatches") stage that defines how the patches were extracted
...
`gt_field` | The ground truth field for the evaluation patches in this view.  
---|---  
`pred_field` | The predictions field for the evaluation patches in this view.  
`app_config` | Dataset-specific settings that customize how this collection is visualized in the [FiftyOne App](user_guide__app.md#fiftyone-app).  
`camera_intrinsics` | The camera intrinsics of the underlying dataset.  
`classes` | The classes of the underlying dataset.  
`dataset_name` | The name of the underlying dataset.  
...
an optional dict defining a [MongoDB read query](https://docs.mongodb.com/manual/tutorial/query-documents/#read-operations-query-argument) that samples must match in order to be included in this view
...
fiftyone.core.patches.make_patches_dataset(_sample_collection_ , _field_ , _other_fields =None_, _keep_label_lists =False_, _include_indexes =False_, _name =None_, _persistent =False_, __generated =False_)#
...
controls whether fields other than `field` and the default sample fields are included. Can be any of the following:
  ...
  * **include_indexes** (_False_) â whether to recreate any custom indexes on `field` and `other_fields` on the new dataset (True) or a list of specific indexes or index prefixes to recreate. By default, no custom indexes are recreated
...
fiftyone.core.patches.make_evaluation_patches_dataset(_sample_collection_ , _eval_key_ , _other_fields =None_, _include_indexes =False_, _name =None_, _persistent =False_, __generated =False_)#
...
controls whether fields other than the ground truth/predicted fields and the default sample fields are included. Can be any of the following:
  ...
  * **include_indexes** (_False_) â whether to recreate any custom indexes on the ground truth/predicted fields and `other_fields` on the new dataset (True) or a list of specific indexes or index prefixes to recreate. By default, no custom indexes are recreated
