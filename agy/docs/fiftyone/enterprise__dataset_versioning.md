---
source_url: https://docs.voxel51.com/enterprise/dataset_versioning.html
---
...
Dataset Versioning allows you to capture the state of your dataset in time so that it can be referenced in the future. This enables workflows like recalling particular important events in the datasetâs lifecycle (model trained, annotation added, etc) as well as helping to prevent accidental data loss.

## Overview#

Dataset Versioning in FiftyOne Enterprise is implemented as a linear sequence of read-only Snapshots. In other words, creating a new Snapshot creates a permanent record of the datasetâs contents that can be loaded and viewed at any time in the future, but not directly edited. Conversely, the current version of a dataset is called its HEAD (think git). If you have not explicitly loaded a Snapshot, you are viewing its HEAD, and you can make additions, updates, and deletions to the datasetâs contents as you normally would (provided you have sufficient permissions).

Snapshots record all aspects of your data stored within FiftyOne, including dataset-level information, schema, samples, frames, brain runs, and evaluations. However, Snapshots exclude any information stored in external services, such as media stored in cloud buckets or embeddings stored in an external vector database, which are assumed to be immutable. If you need to update the image for a sample in a dataset, for example, update the sampleâs filepathâ-which is tracked by snapshotsâ-rather than updating the media in cloud storage in-placeâ-which would not be tracked by snapshots. This design allows dataset snapshots to be as lightweight and versatile as possible.

Dataset-level metadata | Schema, tags, and other metadata  
---|---  
Saved views  
Runs and run results  
Sample-level metadata | All sample metadata (including tags, labels, detections, segmentations, custom fields, etc.)  
All video frame metadata  
  
Media | Images, videos, point clouds  
---|---  
Segmentations or heatmaps stored on disk or in the cloud  
External information | Run results stored in external systems e.g., embeddings stored in a [vector database](integrations__qdrant.md#qdrant-integration)  
  
Dataset Versioning has been built with an extensible architecture so that different versioning backends can be swapped in. Each backend may have different tradeoffs in terms of performance, storage, and deployment needs, so users should be able to choose the best fit for their needs. In addition, many users may already have a versioning solution external to FiftyOne Enterprise, and the goal is to support integration around those use cases as well.
...
A Snapshot whose state and contents are entirely _materialized_ in the MongoDB database. The Snapshot is âready to goâ and be loaded instantly for analysis and visualization.
...
Snapshot your datasets easier knowing your database wonât be overrun!

If your snapshots are important for historical significance but arenât used very often, then you can consider archiving snapshots. This is especially helpful with the internal duplication backend where creating snapshots causes database storage to grow quickly!

When a snapshot is archived, all of its contents are stored in an archive in the configured cold storage location: either a mounted filesystem or cloud storage folder (using your deploymentâs [cloud credentials](enterprise__installation.md#enterprise-cloud-credentials)).
...
If the materialized snapshots per-dataset limit is exceeded, then the snapshot _within the dataset_ that was least-recently loaded will be archived.
...
The table below shows the [dataset permissions](enterprise__roles_and_permissions.md#enterprise-permissions) required to perform different Snapshot-related operations:

Snapshot Operation | User Permissions on Dataset  
---|---  
| Can View | Can Edit | Can Manage  
Browse Snapshot in App | â  | â  | â   
Load Snapshot in SDK | â  | â  | â   
Create Snapshot |  |  | â   
Delete Snapshot |  |  | â   
Archive Snapshot |  |  | â   
Unarchive Snapshot | â  | â  | â   
Revert dataset to Snapshot |  |  | â   
Edit Snapshotâs contents | â | â | â  
  
## Using snapshots#

In contrast to a datasetâs HEAD, Snapshots are **read-only**. When viewing in the App, the UI is similar to interacting with a HEAD dataset, but users will not be able to make any edits to the objects. Similarly, when using the FiftyOne SDK, users will not be able to perform any operation that would trigger a modification to the stored dataset.

### List snapshots#

#### Enterprise UI#

To access the Snapshot history and management page, click the âHistory tabâ on a datasetâs main page.
...
You can also list Snapshot names for a dataset using the [`list_snapshots()`](enterprise__management_sdk.md#fiftyone.management.snapshot.list_snapshots "fiftyone.management.snapshot.list_snapshots") method from the Management SDK.
    
    
    1import fiftyone.management as fom
    2
    3dataset_name = "quickstart"
    4fom.list_snapshots(dataset_name)
    

Then you can get more detailed information on a single Snapshot using the [`get_snapshot_info()`](enterprise__management_sdk.md#fiftyone.management.snapshot.get_snapshot_info "fiftyone.management.snapshot.get_snapshot_info") method.
    ...
    4snapshot_name = "v0.1"
    5
    6fom.get_snapshot_info(dataset, snapshot_name)
    

### Loading snapshots#

Any user with Can View permissions to a dataset can view and load its snapshots via the Enterprise UI or the SDK.

#### Enterprise UI#

From the datasetâs History tab, click the âBrowseâ button next to a Snapshot in the snapshot list to load the Snapshot in the UI.
...
We can also link directly to this Snapshot page by copying the URL from the address bar or from the âShare Datasetâ page which opens from the âShareâ button. For the above Snapshot, it would look like this:
...
Users with Can Manage permissions to a dataset can create Snapshots through the Enterprise UI or the Management SDK.
...
The latest changes summary is not continuously updated; click the âRefreshâ button to recompute these values.

![create-refresh-button](../_images/create-refresh-button.png)

To create a Snapshot, provide a unique name and an optional description, then click the âSave new snapshotâ button.
...
To get the latest changes summary as in the Create snapshot panel, use [`get_dataset_latest_changes_summary()`](enterprise__management_sdk.md#fiftyone.management.snapshot.get_dataset_latest_changes_summary "fiftyone.management.snapshot.get_dataset_latest_changes_summary").
    
    
    1import fiftyone.management as fom
    2
    3fom.get_dataset_latest_changes_summary(dataset.name)
    

To recalculate the latest changes summary as in the Refresh button in that panel, use [`calculate_dataset_latest_changes_summary()`](enterprise__management_sdk.md#fiftyone.management.snapshot.calculate_dataset_latest_changes_summary "fiftyone.management.snapshot.calculate_dataset_latest_changes_summary").
    
    
     1import fiftyone.management as fom
     2
     3old = fom.calculate_dataset_latest_changes_summary(dataset.name)
     4assert old == fom.get_dataset_latest_changes_summary(dataset.name)
     5
     6dataset.delete_samples(dataset.take(5))
     7
     8# Cached summary hasn't been updated
     9assert old == fom.get_dataset_latest_changes_summary(dataset.name)
    10
    11new = fom.calculate_dataset_latest_changes_summary(dataset.name)
    12assert new.updated_at > changes.updated_at
    

To create a new Snapshot, use the [`create_snapshot()`](enterprise__management_sdk.md#fiftyone.management.snapshot.create_snapshot "fiftyone.management.snapshot.create_snapshot") method.
    ...
    3dataset_name = "quickstart"
    4snapshot_name = "v0.1"
    5description = "Version 0.1 in which I have made many awesome changes!"
    6snapshot = fom.create_snapshot(dataset_name, snapshot_name, description)
    

### Deleting a snapshot#

Users with Can Manage permissions to a dataset can delete snapshots through the Enterprise UI or the Management SDK.
...
If the Snapshot is _not_ the most recent, the sample change summary for the following Snapshot will be automatically recalculated based on the previous Snapshot.
...
To delete a Snapshot via the App, open the 3-dot (kebab) menu for the Snapshot. In the menu, click âDelete snapshotâ. This will bring up a confirmation dialog to prevent accidental deletions.

![delete-snapshot](../_images/delete-snapshot.png)

#### SDK#

You can also use the [`delete_snapshot()`](enterprise__management_sdk.md#fiftyone.management.snapshot.delete_snapshot "fiftyone.management.snapshot.delete_snapshot") method in the Management SDK.
    ...
    3dataset = "quickstart"
    4snapshot_name = "v0.1"
    5fom.delete_snapshot(dataset, snapshot_name)
...
To revert a dataset to a Snapshotâs state, click the 3-dot (kebab) menu in the History tab for the Snapshot you want to rollback to and select âRollback to this snapshotâ. This will bring up a confirmation dialog to prevent accidental deletions.

![rollback-snapshot](../_images/rollback-snapshot.png)

#### SDK#

You can also use the [`revert_dataset_to_snapshot()`](enterprise__management_sdk.md#fiftyone.management.snapshot.revert_dataset_to_snapshot "fiftyone.management.snapshot.revert_dataset_to_snapshot") method in the Management SDK.
     ...
     3dataset = "quickstart"
     4snapshot_name = "v0.1"
     5description = "Initial dataset snapshot"
     6fom.create_snapshot(dataset, snapshot_name, description)
     7
     8# Oops we deleted everything!
     9dataset.delete_samples(dataset.values("id"))
    10
    11# Phew!
    12fom.revert_dataset_to_snapshot(dataset.name, snapshot_name)
    13dataset.reload()
    14
    15assert len(dataset) > 0
...
To manually archive a snapshot, click the 3-dot (kebab) menu in the History tab for a snapshot you want to archive and select âArchive snapshotâ. This will begin the archival process and the browse button will be replaced with an âArchivingâ spinnerâ:

![archive-snapshot](../_images/archive-snapshot.png)

#### SDK#

You can also use the [`archive_snapshot()`](enterprise__management_sdk.md#fiftyone.management.snapshot.archive_snapshot "fiftyone.management.snapshot.archive_snapshot") method in the Management SDK:
    
    
    1import fiftyone as fo
    2import fiftyone.management as fom
    3
    4snapshot_name = "v0.1"
    5
    6# We don't use this regularly, archive it!
    7fom.archive_snapshot(dataset.name, snapshot_name)
    8
    9fo.load_dataset(dataset.name, snapshot_name) # throws error, can't load!
...
To unarchive a snapshot, click the âUnarchiveâ button in the History tab for a snapshot you want to unarchive. This will begin the unarchival process and the archive button will be replaced with an âUnarchivingâ spinner:

![unarchive-snapshot](../_images/unarchive-snapshot.png)

#### SDK#

You can also use the [`unarchive_snapshot()`](enterprise__management_sdk.md#fiftyone.management.snapshot.unarchive_snapshot "fiftyone.management.snapshot.unarchive_snapshot") method in the Management SDK:
     ...
     5description = "Initial dataset snapshot"
     6
     7# We don't use this regularly, archive it!
     8fom.archive_snapshot(dataset.name, snapshot_name)
     9fo.load_dataset(dataset.name, snapshot_name) # throws error, can't load!
    10
    11# Oops we need it now, unarchive it!
    12fom.unarchive_snapshot(dataset.name, snapshot_name)
    13fo.load_dataset(dataset.name, snapshot_name) # works now!
    

## Pluggable backends#

Dataset versioning was built with an extensible architecture to support different versioning backend implementations being built and swapped in to better suit the usersâ needs and technology preferences. In the future, this section will contain information and discussion about each of these available backends, including their strengths/limitations and configuration options.

For the initial release in FiftyOne Enterprise v1.4.0, however, there is only one backend choice described below. Additional backends may be implemented in the future, but for now, releasing dataset versioning with the first iteration was prioritized so that users can begin to see value and provide feedback as soon as possible.
...
â  | Simple  
---|---  
â  | Uses existing MongoDB; no extra deployment components  
â  | Browsing/loading is fast because the Snapshots are always materialized  
â  | For a create-then-load workflow, it has the lowest overhead cost of any backend since materialized and virtual forms are one and the same  
  
#### Limitations#

â | Creating a Snapshot takes time proportional to clone dataset  
---|---  
â | Calculating sample change summaries is less efficient  
â | Storage is highly duplicative  
...
As this feature matures, we will have better recommendations for best practices. For now given the limited starting options in the initial iteration, we have the following advice:
  ...
  * Delete old snapshots you donât need anymore.
...
Since Snapshots impact the storage needs of FiftyOne Enterprise, some guard rails have been put in place to control the maximum amount of Snapshots that can be created. If a threshold has been exceeded while a user attempts to create a new Snapshot, they will receive an error informing them that it may be time to remove old Snapshots.
...
---|---|---  
Maximum total Snapshots | `FIFTYONE_SNAPSHOTS_MAX_IN_DB` | 100 | The max total number of Snapshots allowed at once. -1 for no limit. If this limit is exceeded then automatic archival is triggered if enabled, otherwise an error is raised.  
Maximum Snapshots per-dataset | `FIFTYONE_SNAPSHOTS_MAX_PER_DATASET` | 20 | The max number of Snapshots allowed per dataset. -1 for no limit. If this limit is exceeded then automatic archival is triggered if enabled, otherwise an error is raised.  
Snapshot Archive Path | `FIFTYONE_SNAPSHOTS_ARCHIVE_PATH` | `None` | Full path to network-mounted file system or a cloud storage path to use for snapshot archive storage. The default `None` means archival is disabled.  
Automatic Archival Min Age | `FIFTYONE_SNAPSHOTS_MIN_LAST_LOADED_SEC` | 86400 | The minimum last-loaded age in seconds (as defined by `now-last_loaded_at` ) a snapshot must meet to be considered for automatic archival. This limit is intended to help curtail automatic archival of a snapshot a user is actively working with. The default value is 1 day.  
  
## Roadmap#

The following are some items that are on the roadmap for future iterations of the dataset versioning system. Keep an eye out for future FiftyOne Enterprise versions for these additional features!
