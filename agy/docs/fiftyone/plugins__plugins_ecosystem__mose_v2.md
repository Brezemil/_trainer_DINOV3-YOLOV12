---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/mose_v2.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/voxel51/mose-v2)
...
## Notes:#
  ...
  * Extracts `train/` and `valid/` under the FiftyOne-managed dataset directory. A symlink `validation` â `valid` is created when needed so split names match FiftyOneâs expectations.
...
Each image is tagged with its **split** and with its **sequence** name â frames that share a `sequence_id` belong to the same clip.

For a **video-like** browser in the App, use a dynamic grouped view â one group per sequence, frames ordered by `frame_number`.

![MOSEv2 sample visualization \(grid\)](https://raw.githubusercontent.com/voxel51/mose-v2/main/assets/mose-grid.png)

![MOSEv2 grouped / carousel view](https://raw.githubusercontent.com/voxel51/mose-v2/main/assets/mose-carousel.png)
