---
source_url: https://docs.voxel51.com/api/fiftyone.core.brain.html
---
...
`BrainInfo`(key[,Â version,Â timestamp,Â config]) | Information about an brain method that has been run on a dataset.  
---|---  
`BrainMethodConfig`(**kwargs) | Base class for configuring `BrainMethod` instances.  
`BrainMethod`(config) | Base class for brain methods.  
`BrainResults`(samples,Â config,Â key[,Â backend]) | Base class for brain method results.  
  
class fiftyone.core.brain.BrainInfo(_key_ , _version =None_, _timestamp =None_, _config =None_)#
...
Parameters:
  ...
  * **timestamp** (_None_) â the UTC `datetime` when the brain method was run

  * **config** (_None_) â the `BrainMethodConfig` for the run
...
class fiftyone.core.brain.BrainMethod(_config_)#
...
class fiftyone.core.brain.BrainResults(_samples_ , _config_ , _key_ , _backend =None_)#
