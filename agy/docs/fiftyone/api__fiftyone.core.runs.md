---
source_url: https://docs.voxel51.com/api/fiftyone.core.runs.html
---
...
`BaseRunInfo`(key[,Â version,Â timestamp,Â config]) | Information about a run on a dataset.  
---|---  
`BaseRunConfig`(**kwargs) | Base class for configuring `BaseRun` instances.  
`BaseRun`(config) | Base class for methods that can be run on a dataset.  
`BaseRunResults`(samples,Â config,Â key[,Â backend]) | Base class for storing the results of a run.  
`RunInfo`(key[,Â version,Â timestamp,Â config]) | Information about a run on a dataset.  
`RunConfig`(**kwargs) | Class for configuring `Run` instances.  
`Run`(config) | Class for runs.  
`RunResults`(samples,Â config,Â key[,Â backend]) | Class for run results.  
  
class fiftyone.core.runs.BaseRunInfo(_key_ , _version =None_, _timestamp =None_, _config =None_)#
...
Parameters:
  ...
  * **version** (_None_) â the version of FiftyOne when the run was executed

  * **timestamp** (_None_) â the UTC `datetime` of the run

  * **config** (_None_) â the `BaseRunConfig` for the run
...
class fiftyone.core.runs.BaseRunConfig(_** kwargs_)#
...
class fiftyone.core.runs.BaseRun(_config_)#
    

Bases: [`Configurable`](api__fiftyone.core.config.md#fiftyone.core.config.Configurable "fiftyone.core.config.Configurable")
...
a specific run type to match, which can be:
  ...
  * **method** (_None_) â a specific `fiftyone.core.runs.BaseRunConfig.method` string to match
...
class fiftyone.core.runs.BaseRunResults(_samples_ , _config_ , _key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `BaseRunConfig` used

  * **key** â the key for the run

  * **backend** (_None_) â a `BaseRun` instance. If not provided, one is instantiated from `config`
...
class fiftyone.core.runs.RunInfo(_key_ , _version =None_, _timestamp =None_, _config =None_)#
...
Parameters:
  ...
  * **timestamp** (_None_) â the UTC `datetime` when the run was initiated

  * **config** (_None_) â the `RunConfig` for the run
...
class fiftyone.core.runs.RunConfig(_** kwargs_)#
    

Bases: `BaseRunConfig`

Class for configuring `Run` instances.
...
class fiftyone.core.runs.Run(_config_)#
...
class fiftyone.core.runs.RunResults(_samples_ , _config_ , _key_ , _backend =None_, _** kwargs_)#
    

Bases: `BaseRunResults`
...
Parameters:
  ...
  * **config** â the `RunConfig` used

  * **key** â the key for the run

  * **backend** (_None_) â a `Run` instance. If not provided, one is instantiated from `config`

  * ****kwargs** â JSON serializable data to store on the results
