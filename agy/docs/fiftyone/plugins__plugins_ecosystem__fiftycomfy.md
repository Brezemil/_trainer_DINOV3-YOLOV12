---
source_url: https://docs.voxel51.com/plugins/plugins_ecosystem/fiftycomfy.html
---
...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/harpreetsahota204/FiftyComfy)

# FiftyComfy#

![img](https://raw.githubusercontent.com/harpreetsahota204/FiftyComfy/main/fiftycomfy.gif)

A visual, node-based workflow editor for [FiftyOne](https://docs.voxel51.com) â powered by [LiteGraph.js](https://github.com/comfyorg/litegraph.js) (the same engine behind ComfyUI). Build dataset curation, analysis, and model evaluation pipelines by connecting nodes on a canvas, without writing code.
...
## Getting Started#
  ...
  2. Open the **FiftyComfy** panel from the `+` button in the Appâs panel selector.
...
When you click âRun Workflowâ, the graph is serialized to JSON and sent to the Python backend. The executor walks the graph in topological order, passing FiftyOne DatasetView objects between nodes. Output nodes like âSet App Viewâ push the final view into the FiftyOne App grid.
...
**Load Saved View** | Loads a named saved view from the dataset. Dropdown populated from your datasetâs saved views.  
...
---|---|---  
**Match** | Filter samples by a ViewField expression | `expression` (text) â e.g., `F('confidence') > 0.5`  
...
**Map Labels** | Remap label values using a mapping dict | `field` (dropdown), `map` (text) â e.g., `{"cat": "animal"}`  
...
Node | Description | Key Widgets  
---|---|---  
**Apply Zoo Model** | Run inference with a zoo model | `model` (dropdown â 17 popular models), `label_field` (text), `confidence_thresh` (number), `store_logits` (toggle)  
...
Field dropdowns are automatically populated from your datasetâs schema when the panel loads:
...
## Saving and Loading Workflows#
  ...
  * **Save** : Click the âSaveâ button in the toolbar and enter a name. Workflows are stored in browser localStorage.

  * **Load** : Click âLoadâ to see saved workflows and click one to restore it.

  * **Clear** : Click âClearâ to reset the canvas.
