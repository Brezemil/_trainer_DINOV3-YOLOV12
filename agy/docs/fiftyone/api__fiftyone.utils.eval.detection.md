---
source_url: https://docs.voxel51.com/api/fiftyone.utils.eval.detection.html
---
...
---|---  
`DetectionEvaluation`(config) | Base class for detection evaluation methods.  
`DetectionResults`(samples,Â config,Â eval_key,Â ...) | Class that stores the results of a detection evaluation.  
  
fiftyone.utils.eval.detection.evaluate_detections(_samples_ , _pred_field_ , _gt_field ='ground_truth'_, _eval_key =None_, _classes =None_, _missing =None_, _method =None_, _iou =0.5_, _use_masks =False_, _use_boxes =False_, _classwise =True_, _dynamic =True_, _custom_metrics =None_, _progress =None_, _** kwargs_)#
...
class fiftyone.utils.eval.detection.DetectionEvaluationConfig(_pred_field_ , _gt_field_ , _iou =None_, _classwise =None_, _custom_metrics =None_, _** kwargs_)#
...
class fiftyone.utils.eval.detection.DetectionEvaluation(_config_)#
...
**config** â a `DetectionEvaluationConfig`

**Methods:**

`register_samples`(samples,Â eval_key[,Â dynamic]) | Registers the collection on which evaluation will be performed.  
---|---  
`evaluate`(doc[,Â eval_key]) | Evaluates the ground truth and predictions in the given document.  
...
evaluate(_doc_ , _eval_key =None_)#
...
Parameters:
  ...
  * **doc** â a [`fiftyone.core.document.Document`](api__fiftyone.core.document.md#fiftyone.core.document.Document "fiftyone.core.document.Document")
...
class fiftyone.utils.eval.detection.DetectionResults(_samples_ , _config_ , _eval_key_ , _matches_ , _classes =None_, _missing =None_, _custom_metrics =None_, _backend =None_)#
