---
source_url: https://docs.voxel51.com/api/fiftyone.utils.sam3.html
---

# fiftyone.utils.sam3#

[Segment Anything 3](https://github.com/facebookresearch/sam3) wrapper for the FiftyOne Model Zoo.
...
`SegmentAnything3ImageModelConfig`(cfg_dict) | Configuration for running a `SegmentAnything3ImageModel`.  
---|---  
`SegmentAnything3ImageGetItem`([...]) | A `GetItem` that loads images, bounding boxes and/or keypoints to feed to `SegmentAnything3ImageModel` instances.  
`SegmentAnything3ImageGetItemForVideo`([...]) | Workaround for applying image model to video reader frames.  
`SAM3ConceptSegmenterOutputProcessor`([...]) | Converts SAM model outputs to FiftyOne format.  
`SegmentAnything3ImageModel`(config) | Wrapper for running [Segment Anything 3](https://ai.meta.com/research/sam3) inference.  
  
**Functions:**

`build_sam_datapoint_transform`() | Builds transforms for SAM3 datapoints.  
---|---  
  
class fiftyone.utils.sam3.SegmentAnything3ImageModelConfig(_cfg_dict_)#
...
Configuration for running a `SegmentAnything3ImageModel`.
...
Parameters:
  ...
  * **operation_mode** (_"concept"_) â concept or visual mode of operation for inference
...
class fiftyone.utils.sam3.SegmentAnything3ImageGetItem(_field_mapping =None_, _transform =None_, _use_numpy =False_, _box_transform =None_, _point_transform =None_, _text_prompts =None_, _operation_mode ='concept'_, _** kwargs_)#
...
A `GetItem` that loads images, bounding boxes and/or keypoints to feed to `SegmentAnything3ImageModel` instances.
...
Parameters:
  ...
  * **text_prompts** (_None_) â Text prompts for concept prompting the model

  * **operation_mode** (_"concept"_) â Operation mode of the model (required for collate_fn)
...
class fiftyone.utils.sam3.SegmentAnything3ImageGetItemForVideo(_field_mapping =None_, _transform =None_, _use_numpy =False_, _box_transform =None_, _point_transform =None_, _text_prompts =None_, _operation_mode ='concept'_, _** kwargs_)#
    

Bases: `SegmentAnything3ImageGetItem`
...
class fiftyone.utils.sam3.SAM3ConceptSegmenterOutputProcessor(_classes =None_, _mask_thresh =0.5_)#
...
class fiftyone.utils.sam3.SegmentAnything3ImageModel(_config_)#
    

Bases: [`SegmentAnything2ImageModel`](api__fiftyone.utils.sam2.md#fiftyone.utils.sam2.SegmentAnything2ImageModel "fiftyone.utils.sam2.SegmentAnything2ImageModel")

Wrapper for running [Segment Anything 3](https://ai.meta.com/research/sam3) inference.

Parameters:
    

**config** â a `SegmentAnything3ModelConfig`

**Attributes:**

`operation_mode` | Whether to use the model in visual or concept segmentation mode  
---|---  
...
`collate_fn`(batch) | Collates a batch of inputs where each input is generated from `SegmentAnything3ImageGetItem`.  
---|---  
`build_get_item`([field_mapping]) | Builds a `SegmentAnything3ImageGetItem` for loading model input from samples.  
...
Collates a batch of inputs where each input is generated from `SegmentAnything3ImageGetItem`.

Parameters:
    

**batch** â a list of dict containing model input from `SegmentAnything3ImageGetItem`
...
Builds a `SegmentAnything3ImageGetItem` for loading model input from samples.
...
a `SegmentAnything3ImageGetItem` instance
