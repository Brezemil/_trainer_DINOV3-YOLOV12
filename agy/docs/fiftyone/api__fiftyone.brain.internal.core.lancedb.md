---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.lancedb.html
---
...
`LanceDBSimilarityConfig`([table_name,Â ...]) | Configuration for a LanceDB similarity instance.  
---|---  
`LanceDBSimilarity`(config) | LanceDB similarity factory.  
`LanceDBSimilarityIndex`(samples,Â config,Â ...) | Class for interacting with LanceDB similarity indexes.  
  
class fiftyone.brain.internal.core.lancedb.LanceDBSimilarityConfig(_table_name =None_, _metric ='cosine'_, _uri ='/tmp/lancedb'_, _** kwargs_)#
...
Parameters:
  ...
  * **table_name** (_None_) â the name of the LanceDB table to use. If none is provided, a new table will be created

  * **metric** (_"cosine"_) â the embedding distance metric to use when creating a new index. Supported values are `("cosine", "euclidean")`

  * **uri** (_"/tmp/lancedb"_) â the database URI to use

  * ****kwargs** â keyword arguments for `SimilarityConfig`
...
`load_credentials`([uri]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
class fiftyone.brain.internal.core.lancedb.LanceDBSimilarity(_config_)#
...
**config** â a `LanceDBSimilarityConfig`
...
class fiftyone.brain.internal.core.lancedb.LanceDBSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `LanceDBSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `LanceDBSimilarity` instance
...
`table` | The `lancedb.LanceTable` instance for this index.  
---|---  
