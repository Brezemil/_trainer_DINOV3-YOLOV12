---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.mosaic.html
---
...
`MosaicSimilarityConfig`([endpoint_name,Â ...]) | Configuration for the Mosaic similarity backend.  
---|---  
`MosaicSimilarity`(config) | Mosaic similarity factory.  
`MosaicSimilarityIndex`(samples,Â config,Â brain_key) | Class for interacting with Mosaic similarity indexes.  
  
class fiftyone.brain.internal.core.mosaic.MosaicSimilarityConfig(_endpoint_name =None_, _workspace_url =None_, _catalog_name =None_, _schema_name =None_, _index_name =None_, _service_principal_client_id =None_, _service_principal_client_secret =None_, _personal_access_token =None_, _** kwargs_)#
...
Parameters:
  ...
  * **endpoint_name** (_None_) â the name of the vector search endpoint that was created in the Databricks workspace

  * **workspace_url** (_None_) â the URL of the Databricks workspace

  * **catalog_name** (_None_) â the name of the catalog in the Databricks workspace

  * **schema_name** (_None_) â the name of the schema in the Databricks workspace

  * **index_name** (_None_) â the name of the index to use, if one is not provided, a unique name will be generated

  * **service_principal_client_id** (_None_) â the client ID of the service principal created for authentication

  * **service_principal_client_secret** (_None_) â the client secret of the service principal created for authentication

  * **personal_access_token** (_None_) â the personal access token created for authentication
...
`load_credentials`([workspace_url,Â ...]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
property workspace_url#
    

property service_principal_client_id#
    

property service_principal_client_secret#
...
load_credentials(_workspace_url =None_, _service_principal_client_id =None_, _service_principal_client_secret =None_, _personal_access_token =None_)#
...
class fiftyone.brain.internal.core.mosaic.MosaicSimilarity(_config_)#
...
**config** â a `MosaicSimilarityConfig`
...
class fiftyone.brain.internal.core.mosaic.MosaicSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `MosaicSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `MosaicSimilarity` instance
...
`client` | The `databricks.vector_search.client.VectorSearchClient` instance for this index.  
---|---  
...
add_to_index(_embeddings_ , _sample_ids_ , _label_ids =None_, _overwrite =True_, _allow_existing =True_, _warn_existing =False_, _reload =True_, _batch_size =200_)#
