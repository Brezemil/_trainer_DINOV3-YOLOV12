---
source_url: https://docs.voxel51.com/api/fiftyone.utils.eval.segmentation.html
---
...
---|---  
`SegmentationEvaluation`(config) | Base class for segmentation evaluation methods.  
`SimpleEvaluationConfig`(pred_field,Â gt_field) | Class for configuring `SimpleEvaluation` instances.  
`SimpleEvaluation`(config) | Standard pixelwise segmentation evaluation.  
`SegmentationResults`(samples,Â config,Â ...[,Â ...]) | Class that stores the results of a segmentation evaluation.  
  
fiftyone.utils.eval.segmentation.evaluate_segmentations(_samples_ , _pred_field_ , _gt_field ='ground_truth'_, _eval_key =None_, _mask_targets =None_, _method =None_, _custom_metrics =None_, _progress =None_, _** kwargs_)#
...
class fiftyone.utils.eval.segmentation.SegmentationEvaluationConfig(_pred_field_ , _gt_field_ , _compute_dice =False_, _custom_metrics =None_, _** kwargs_)#
...
class fiftyone.utils.eval.segmentation.SegmentationEvaluation(_config_)#
...
**config** â a `SegmentationEvaluationConfig`

**Methods:**

`register_samples`(samples,Â eval_key) | Registers the collection on which evaluation will be performed.  
---|---  
`evaluate_samples`(samples[,Â eval_key,Â ...]) | Evaluates the predicted segmentation masks in the given samples with respect to the specified ground truth masks.  
...
evaluate_samples(_samples_ , _eval_key =None_, _mask_targets =None_, _progress =None_)#
...
Parameters:
  ...
  * **mask_targets** (_None_) â a dict mapping mask values to labels. May contain a subset of the possible classes if you wish to evaluate a subset of the semantic classes. By default, the observed pixel values are used as labels
...
class fiftyone.utils.eval.segmentation.SimpleEvaluationConfig(_pred_field_ , _gt_field_ , _compute_dice =False_, _bandwidth =None_, _average ='micro'_, _custom_metrics =None_, _** kwargs_)#
    

Bases: `SegmentationEvaluationConfig`
...
Parameters:
  ...
  * **bandwidth** (_None_) â an optional bandwidth along the contours of the ground truth masks to which to restrict attention when computing accuracies. A typical value for this parameter is 5 pixels. By default, the entire masks are evaluated

  * **average** (_"micro"_) â the averaging strategy to use when populating precision and recall numbers on each sample
...
class fiftyone.utils.eval.segmentation.SimpleEvaluation(_config_)#
...
class fiftyone.utils.eval.segmentation.SegmentationResults(_samples_ , _config_ , _eval_key_ , _pixel_confusion_matrix_ , _classes_ , _matches =None_, _missing =None_, _custom_metrics =None_, _backend =None_)#
