---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.pgvector.html
---
...
`PgVectorSimilarityConfig`([index_name,Â ...]) | Configuration for the PGVector similarity backend.  
---|---  
`PgVectorSimilarity`(config) | PGVector similarity factory.  
`PgVectorSimilarityIndex`(samples,Â config,Â ...) | Class for interacting with PGVector similarity indexes.  
  
class fiftyone.brain.internal.core.pgvector.PgVectorSimilarityConfig(_index_name =None_, _table_name =None_, _metric ='cosine'_, _connection_string =None_, _ssl_cert =None_, _ssl_key =None_, _ssl_root_cert =None_, _work_mem ='64MB'_, _hnsw_m =16_, _hnsw_ef_construction =64_, _** kwargs_)#
...
Parameters:
  ...
  * **index_name** (_None_) â the name of the PGVector index to use or create. If none is provided, a default index name will be used.

  * **table_name** (_None_) â the name of the table to use or create. If none is provided, a default table name will be used.

  * **metric** (_"cosine"_) â the similarity metric to use. Supported values are `("cosine", "dotproduct", "euclidean", "l1", "jaccard", "hamming")`

  * **connection_string** (_None_) â the connection string to the PostgreSQL database

  * **ssl_cert** (_None_) â the path to the SSL certificate file

  * **ssl_key** (_None_) â the path to the secret key used for the client certificate

  * **ssl_root_cert** (_None_) â the path to the file containing SSL certificate authority (CA) certificate(s).

  * **work_mem** (_"64MB"_) â the base maximum amount of memory to be used by a query operation (such as a sort or hash table) before writing to temporary disk files

  * **hnsw_m** (_16_) â the max number of connections per layer in the HNSW index

  * **hnsw_ef_construction** (_64_) â the size of the dynamic candidate list for constructing the graph for the HNSW index
...
`load_credentials`([connection_string]) | Loads any necessary credentials from the given keyword arguments or the relevant FiftyOne config.  
---|---  
...
load_credentials(_connection_string =None_)#
...
class fiftyone.brain.internal.core.pgvector.PgVectorSimilarity(_config_)#
...
**config** â a `PgVectorSimilarityConfig`
...
class fiftyone.brain.internal.core.pgvector.PgVectorSimilarityIndex(_samples_ , _config_ , _brain_key_ , _backend =None_)#
...
Parameters:
  ...
  * **config** â the `PGVectorSimilarityConfig` used

  * **brain_key** â the brain key

  * **backend** (_None_) â a `PGVectorSimilarity` instance
...
`close_connections`() |   
`get_embeddings_by_id`([sample_ids,Â label_ids]) |   
`get_embeddings`([sample_ids,Â label_ids,Â ...]) | Retrieves the embeddings for the given IDs from the index.  
`cleanup`([drop_table]) | Clean up the database by dropping the HNSW index and optionally the embeddings table.  
`attributes`() | Returns the list of class attributes that will be serialized by `serialize()`.  
...
add_to_index(_embeddings_ , _sample_ids_ , _label_ids =None_, _overwrite =True_, _allow_existing =True_, _warn_existing =False_, _reload =True_, _batch_size =5000_, _close_conn =True_)#
...
get_embeddings_by_id(_sample_ids =None_, _label_ids =None_)#
