...
The notebook walks through:
  ...
  5. Comparing results and recreating the winning strategyâs training data
...
All operators have interactive UIs in the FiftyOne App:
  ...
  3. Select a W&B operator:
     ...
     * **âW &B: Save View as Artifactâ** \- Log training data

     * **âW &B: Log Model Predictionsâ** \- Log inference results

     * **âW &B: Load View from Artifactâ** \- Recreate views

     * **âShow W &B Runâ** \- View training curves alongside your data

     * **âShow W &B Reportâ** \- Embed reports in the panel
...
> **Future plans:** Weâre working on adding support for Hugging Face SFT Trainer workflows. Contributions welcome!
...
### âFIFTYONE_WANDB_API_KEY not setâ#
...
### âwandb is not installedâ#
    
    
    pip install wandb
    

### âCould not find W&B runâ#
...
For large datasets with `include_labels=True`, the operator automatically delegates to a background process. Youâll see a progress bar in the FiftyOne App.
...
Have a training workflow youâd like to add? PRs are welcome! The YOLO operators serve as examples of how to integrate your own fine-tuning pipelines with full W&B tracking.
