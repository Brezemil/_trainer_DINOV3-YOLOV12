---
source_url: https://docs.voxel51.com/api/fiftyone.brain.internal.core.leaky_splits.html
---
...
`compute_leaky_splits`(samples,Â splits[,Â ...]) | See `fiftyone/brain/__init__.py`.  
---|---  
  
**Classes:**

`LeakySplitsConfig`([splits,Â ...]) |   
---|---  
`LeakySplits`(config) |   
`LeakySplitsIndex`(samples,Â config,Â ...) |   
  
fiftyone.brain.internal.core.leaky_splits.compute_leaky_splits(_samples_ , _splits_ , _threshold =None_, _roi_field =None_, _embeddings =None_, _similarity_index =None_, _model =None_, _model_kwargs =None_, _force_square =False_, _alpha =None_, _batch_size =None_, _num_workers =None_, _skip_failures =True_, _progress =None_)#
    

See `fiftyone/brain/__init__.py`.

class fiftyone.brain.internal.core.leaky_splits.LeakySplitsConfig(_splits =None_, _embeddings_field =None_, _similarity_index =None_, _model =None_, _model_kwargs =None_, _** kwargs_)#
...
class fiftyone.brain.internal.core.leaky_splits.LeakySplits(_config_)#
...
`initialize`(samples,Â similarity_index,Â ...) |   
---|---  
`get_fields`(samples,Â _) | Gets the fields that were involved in the given run.  
...
initialize(_samples_ , _similarity_index_ , _split_views_)#
    

get_fields(_samples_ , ___)#
...
class fiftyone.brain.internal.core.leaky_splits.LeakySplitsIndex(_samples_ , _config_ , _similarity_index_ , _split_views_)#
...
`split_views` | A dict mapping split names to views.  
---|---  
`thresh` | The threshold used by the last call to `find_leaks()`.  
`leak_ids` | The list of leaky sample IDs from the last call to `find_leaks()`.  
...
`find_leaks`(thresh) | Scans the index for leaks between splits.  
---|---  
`leaks_view`() | Returns a view containg all potential leaks generated by the last call to `find_leaks()`.  
`leaks_for_sample`(sample_or_id) | Returns a view that contains all leaks related to the given sample.  
`no_leaks_view`([view]) | Returns a view with leaks excluded.  
`tag_leaks`([tag]) | Tags all potential leaks in `leaks_view()` with the given tag.  
`attributes`() | Returns the list of class attributes that will be serialized by `serialize()`.  
...
The threshold used by the last call to `find_leaks()`.

property leak_ids#
    

The list of leaky sample IDs from the last call to `find_leaks()`.
...
**thresh** â the similarity distance threshold to use when detecting potential leaks

leaks_view()#
    

Returns a view containg all potential leaks generated by the last call to `find_leaks()`.
...
leaks_for_sample(_sample_or_id_)#
    

Returns a view that contains all leaks related to the given sample.

The given sample is always first in the returned view, followed by any related leaks.

Parameters:
    

**sample_or_id** â a [`fiftyone.core.sample.Sample`](api__fiftyone.utils.data.md#fiftyone.utils.data.Sample "fiftyone.core.sample.Sample") or sample ID
...
**view** (_None_) â an optional [`fiftyone.core.view.DatasetView`](api__fiftyone.core.view.md#fiftyone.core.view.DatasetView "fiftyone.core.view.DatasetView") from which to exclude. By default, `samples()` is used

tag_leaks(_tag ='leak'_)#
    

Tags all potential leaks in `leaks_view()` with the given tag.

Parameters:
    

**tag** (_"leak"_) â the tag string to apply
