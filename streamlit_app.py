...
# Page configuration
st.set_page_config(
    page_title="YOLOv12-DINO Object Detection", page_icon="🎯", layout="wide", initial_sidebar_state="expanded"
)
    ...
    try:
        ...
        with st.spinner("Loading YOLOv12-DINO model..."):
            st.session_state.model_instance = YOLOInference(
                weights=weights_path, conf=0.25, iou=0.7, imgsz=640, device=device, verbose=True
            )
...
def perform_detection(
    image: np.ndarray, conf_threshold: float, iou_threshold: float, image_size: int
...
def process_detection_results(result, inference_time: float) -> Dict:
    ...
    if result.boxes is None or len(result.boxes) == 0:
        return {"total_detections": 0, "class_counts": {}, "detections": [], "inference_time": inference_time}
...
def create_detection_chart(detection_data: Dict):
    ...
    fig_pie = px.pie(
        values=list(class_counts.values()), names=list(class_counts.keys()), title="Detection Distribution by Class"
    )
    ...
    fig_conf = px.box(x=classes, y=confidences, title="Confidence Distribution by Class")
...
def main():
    ...
    with st.sidebar:
        ...
        device = st.selectbox("Device", options=["cpu", "cuda", "mps"], index=0, help="Select computation device")
        ...
        image_size = st.selectbox(
            "Image Size", options=[320, 416, 512, 640, 832, 1024, 1280], index=3, help="Input image size for model"
        )
    ...
    with col1:
        ...
        image_file = st.file_uploader(
            "Upload Image", type=["jpg", "jpeg", "png", "bmp"], help="Upload an image for object detection"
        )
    ...
    if (
        "last_detection" in st.session_state
        and st.session_state.last_detection["detection_data"]["total_detections"] > 0
    ):
