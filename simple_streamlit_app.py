...
# Page configuration
st.set_page_config(page_title="YOLOv12-DINO Object Detection", page_icon="🎯", layout="wide")
...
def perform_detection(image_np, conf_threshold, iou_threshold, image_size):
    ...
    try:
        ...
        try:
            ...
            results = st.session_state.model_instance.predict_single(source=tmp_path, save=False, show=False)
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "Upload Model Weights (.pt file)", type=["pt"], help="Upload your trained YOLOv12-DINO model weights"
        )
        ...
        iou_threshold = st.slider("IoU Threshold", min_value=0.01, max_value=1.0, value=0.7, step=0.01)

        image_size = st.selectbox("Image Size", options=[320, 416, 512, 640, 832, 1024], index=3)
